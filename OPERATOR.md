...
The app deploy contract supports `AUTH_MODE=builtin_admin` and `AUTH_MODE=oidc`. Built-in admin keeps generated empty OIDC placeholders filtered out. OIDC requires non-empty `OIDC_ISSUER_URL`, `OIDC_CLIENT_ID`, and `OIDC_CLIENT_SECRET`; public OIDC values render into app config and `OIDC_CLIENT_SECRET` renders into the app Secret. Contract errors name keys without printing secret values.
