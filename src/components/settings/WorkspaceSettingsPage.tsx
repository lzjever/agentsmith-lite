...
function WorkspaceSettings({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout contentWidth="narrow" header={<PageHeader title="Workspace settings" subtitle="Workspace metadata and project administration." />}>
    ...
    {state === "ready" && data ? <>
      ...
      <form onSubmit={submit} className="space-y-5 border-y border-subtle py-5"><label className="grid gap-2 text-sm"><span>Workspace name</span><Input required name="name" maxLength={160} value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} disabled={mutationBusy||!data.capabilities.canManageSettings||!isActive} /></label><p className="text-sm text-secondary">Project access is managed from each project.</p>{data.capabilities.canManageSettings&&isActive ? <div className="flex justify-end"><Button type="submit" disabled={mutationBusy || !settingsDirty || !workspaceName.trim()}><Save size={16} />{saving ? "Saving..." : "Save workspace"}</Button></div> : <p className="text-sm text-secondary">Read-only access.</p>}</form>
