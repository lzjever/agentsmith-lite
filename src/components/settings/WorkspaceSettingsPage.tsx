...
export function WorkspaceSettingsPage({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout contentWidth="narrow" header={<PageHeader title="Workspace settings" subtitle="Workspace metadata and project administration." />}>
    ...
    {state === "ready" && data ? <>
      ...
      {canManageLifecycle?<section className="mt-8 border-t border-subtle pt-6"><h2 className="type-title">Lifecycle</h2><p className="mt-1 text-sm text-secondary">Archived workspaces remain available for viewing.</p><Button className="mt-4" variant="outline" disabled={lifecycleBusy} onClick={()=>void setArchive()}><Archive size={16}/>{archived?"Unarchive workspace":"Archive workspace"}</Button></section>:null}
