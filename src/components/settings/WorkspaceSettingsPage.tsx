...
import { Banner, Button, DialogHeader, Heading, Layout, LayoutContent, LayoutFooter, Selector, Spinner, Text, TextInput, useToast } from "@astryxdesign/core";
...
function WorkspaceSettings({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout contentWidth="narrow" header={<PageHeader title="Workspace settings" subtitle="Workspace metadata and project administration." />}>
    ...
    {state === "ready" && data ? <>
      ...
      {canArchive?<Dialog isOpen={archiveOpen} onOpenChange={setArchiveDialogOpen} purpose="form" role="alertdialog" width="min(32rem, calc(100vw - 2rem))" aria-labelledby="workspace-archive-title" aria-describedby="workspace-archive-description"><Layout defaultHasDividers header={<DialogHeader id="workspace-archive-title" title="Archive workspace" />} content={<LayoutContent><form id="workspace-archive-form" onSubmit={(event)=>{event.preventDefault();void setArchive();}}><div className="grid gap-4"><Text id="workspace-archive-description" as="p" display="block" color="secondary">Projects and workspace data remain available for viewing, but changes are disabled until the owner restores this workspace.</Text>{archiveError?<Banner status="error" title="Workspace could not be archived" description={archiveError}/>:null}</div></form></LayoutContent>} footer={<LayoutFooter><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button label="Cancel" type="button" variant="ghost" size="lg" isDisabled={lifecycleBusy} onClick={()=>setArchiveDialogOpen(false)}/><Button label={lifecycleBusy?"Archiving":"Archive workspace"} type="submit" form="workspace-archive-form" variant="destructive" size="lg" isDisabled={lifecycleBusy} isLoading={lifecycleBusy}/></div></LayoutFooter>} /></Dialog>:null}
