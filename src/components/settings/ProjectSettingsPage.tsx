  ...
  return <PageLayout contentWidth="narrow" header={<PageHeader title="Project settings" subtitle="Project metadata and access administration." actions={<Link className="inline-flex items-center gap-2 text-sm text-secondary hover:text-foreground" href={`/workspaces/${workspaceId}/projects/${projectId}/members`}><Users size={16} />Manage members</Link>} />}>
    ...
    {state === "ready" && data ? <>
      ...
      <form onSubmit={submit} className="grid gap-5 border-y border-subtle py-5">
        <label className="grid gap-2 text-sm"><span>Project name</span><Input required name="name" maxLength={160} value={projectName} onChange={(event) => setProjectName(event.target.value)} disabled={mutationBusy||!data.capabilities.canManageSettings||!isActive} /></label>
      ...
      {isOwner?<section className="mt-8 border-t border-subtle pt-6"><h2 className="type-title">Transfer ownership</h2><p className="mt-1 text-sm text-secondary">Choose an existing project member as the new owner.</p>{memberState==="loading"?<p className="mt-4 text-sm text-secondary" role="status">Loading eligible project members...</p>:null}{memberState==="error"?<div className="mt-4 flex items-center justify-between gap-3 border border-error/30 bg-error/10 px-3 py-2" role="alert"><span className="text-sm text-error">Project members could not be loaded.</span><Button label="Retry" variant="ghost" size="sm" aria-label="Retry member loading" icon={<RefreshCw size={14}/>} isDisabled={mutationBusy} onClick={()=>void loadMembers()} /></div>:null}{memberState==="ready"&&ownerCandidates.length === 0 ? <p className="mt-4 text-sm text-secondary" role="status">There are no other project members eligible to become owner.</p> : null}{memberState==="ready"&&ownerCandidates.length>0?<div className="mt-4 flex flex-wrap items-end gap-2"><div className="grid w-64 gap-2"><Label htmlFor="project-owner-target">New project owner</Label><Selector id="project-owner-target" label="New project owner" isLabelHidden options={ownerCandidates.map((member) => ({ value: member.userId, label: memberLabel(member) }))} value={ownerTargetEligible?ownerTarget:""} onChange={setOwnerTarget} placeholder="Select a member" isDisabled={!canTransferOwnership||mutationBusy} size="lg" /></div><Button label="Transfer ownership" variant="secondary" isDisabled={!ownerTarget||!ownerTargetEligible||!canTransferOwnership||mutationBusy} onClick={()=>setOwnerOpen(true)} /></div>:null}</section>:null}
