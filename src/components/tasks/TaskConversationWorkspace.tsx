...
export function TaskConversationWorkspace({ taskId, presentation, onPresentationChange, onProjectionChange, onUnavailable, onArtifactPublished }: { taskId: string; presentation:TaskDetail; onPresentationChange:(presentation:TaskDetail)=>void; onProjectionChange: () => void; onUnavailable?: () => void; onArtifactPublished: () => void }) {
  ...
  if (!snapshot) {
    ...
    return <section className="grid h-full min-h-0 flex-1 place-items-center border border-border bg-muted px-5">{error ? <div className="max-w-md text-center"><Banner status="error" icon={<CircleAlert size={16} />} title="Conversation could not be loaded" description={error} />{canRetryRuntime?<Button className="mt-4" label="Retry" icon={<RefreshCw size={14} />} variant="ghost" size="md" onClick={retry} />:null}</div> : <Text type="supporting" color="secondary">Loading conversation...</Text>}</section>;
  }
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-hidden border border-border bg-muted" aria-label="Task conversation workspace"><TaskRunStatus currentTurn={currentTurn} sandboxState={sandboxState} capabilities={capabilities} aborting={aborting} onAbort={abort} /><TaskConnectionNotice connection={connection} historyStatus={snapshot.historyStatus} runtimeReachability={snapshot.runtimeReachability} runtimeAvailable={runtimeAvailable} error={error} onRetry={retry} />{previewUnavailable && runtimeAvailable && (connection === "connected" || connection === "recovered") ? <TaskPreviewNotice message={previewUnavailable} onRetry={retry} /> : null}<div ref={viewport} className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5" onScroll={onScroll}>{snapshot.hasMoreBefore ? <div className="mb-4 text-center">{historyError ? <Banner className="mb-2 text-left" status="error" title="Earlier messages unavailable" description={historyError} /> : null}<Button label={loadingEarlier ? "Loading..." : "Load earlier messages"} variant="ghost" size="md" isDisabled={loadingEarlier} onClick={() => void loadEarlier()} /></div> : null}<TaskInteractionList taskId={taskId} items={items} preview={preview} allowStopWork={capabilities.stopWork} onStopWork={stopWork} /></div>{newActivity ? <div className="shrink-0 border-t border-border bg-surface py-2 text-center"><Button label="New activity" icon={<ChevronUp size={14} />} variant="secondary" size="md" onClick={showNewActivity} /></div> : null}<TaskComposer capabilities={capabilities} queuedMessages={snapshot.queuedMessages} busy={aborting} unavailableMessage={unavailableMessage} onSend={send} onUpdateQueued={updateQueued} onDeleteQueued={deleteQueued} /></section>;
}
