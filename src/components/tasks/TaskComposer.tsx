...
import { Banner, Button, DialogHeader, IconButton, Layout, LayoutContent, LayoutFooter, Text, TextArea } from "@astryxdesign/core";
...
export function TaskComposer({ capabilities, queuedMessages, busy, unavailableMessage = "Messaging is unavailable", onSend, onUpdateQueued, onDeleteQueued }: { capabilities: TaskCapabilities; queuedMessages: TaskQueuedMessage[]; busy: boolean; unavailableMessage?: string; onSend: (content: string) => Promise<void>; onUpdateQueued: (messageId: string, content: string) => Promise<void>; onDeleteQueued: (messageId: string) => Promise<void> }) {
  ...
  return <section ref={composer} className="shrink-0 border-t border-border bg-surface px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 sm:px-5" aria-label="Task message composer">
    ...
    <Dialog isOpen={Boolean(editing)} onOpenChange={(open) => { if (!open && !messageBusy) closeEdit(); }} purpose="form" width="min(34rem, calc(100vw - 2rem))" aria-label="Edit queued message"><Layout defaultHasDividers header={<DialogHeader title="Edit queued message" subtitle="Only messages the server still accepts can be changed." onOpenChange={(open) => { if (!open && !messageBusy) closeEdit(); }} />} content={<LayoutContent><form id="queued-message-edit-form" onSubmit={(event) => { event.preventDefault(); void saveEdit(); }}><div className="grid gap-4">{editError ? <Banner status="error" title="Message could not be updated" description={editError} /> : null}<TextArea label="Queued message" isLabelHidden value={editDraft} onChange={setEditDraft} isDisabled={messageBusy} rows={5} hasAutoFocus width="100%" /></div></form></LayoutContent>} footer={<LayoutFooter><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" label="Cancel" variant="ghost" size="lg" isDisabled={messageBusy} onClick={closeEdit} /><Button type="submit" form="queued-message-edit-form" label={saving ? "Saving..." : "Save message"} size="lg" isDisabled={messageBusy || !nextEdit || !editChanged} /></div></LayoutFooter>} /></Dialog>
    <Dialog isOpen={Boolean(removing)} onOpenChange={(open) => { if (!open && !deleting) closeRemove(); }} purpose="form" role="alertdialog" width="min(32rem, calc(100vw - 2rem))" aria-labelledby="queued-message-delete-title" aria-describedby="queued-message-delete-description">
      <Layout defaultHasDividers header={<DialogHeader id="queued-message-delete-title" title="Delete queued message?" />} content={<LayoutContent><div className="grid gap-4"><Text id="queued-message-delete-description" as="p" display="block" color="secondary">This message has not started delivery and can be removed.</Text>{removeError ? <Banner status="error" title="Queued message could not be deleted" description={removeError} /> : null}</div></LayoutContent>} footer={<LayoutFooter><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button label="Cancel" variant="ghost" size="lg" isDisabled={deleting} onClick={closeRemove} /><Button label={deleting ? "Deleting" : removeError ? "Try delete again" : "Delete message"} variant="destructive" size="lg" isDisabled={deleting} isLoading={deleting} onClick={() => void remove()} /></div></LayoutFooter>} />
    </Dialog>
  </section>;
}
