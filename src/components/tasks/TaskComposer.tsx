...
import { Button, Dialog, DialogHeader, TextArea } from "@astryxdesign/core";
...
export function TaskComposer({ capabilities, queuedMessages, busy, unavailableMessage = "Messaging is unavailable", onSend, onUpdateQueued, onDeleteQueued }: { capabilities: TaskCapabilities; queuedMessages: TaskQueuedMessage[]; busy: boolean; unavailableMessage?: string; onSend: (content: string) => Promise<void>; onUpdateQueued: (messageId: string, content: string) => Promise<void>; onDeleteQueued: (messageId: string) => Promise<void> }) {
  ...
  return <section className="shrink-0 border-t border-border bg-background px-4 py-4 sm:px-5" aria-label="Task message composer">
    {queuedMessages.length ? <ul className="mb-3 max-h-36 overflow-y-auto divide-y divide-border border-y border-border">{queuedMessages.map((message) => <li key={message.id} className="flex min-w-0 items-start justify-between gap-3 py-2.5"><div className="min-w-0"><p className="break-words text-sm text-foreground">{message.content}</p><p className="mt-1 font-mono text-[13px] tracking-normal text-tertiary">{readableStatus(message.deliveryStatus)}</p>{message.safeError ? <p className="mt-1 break-words text-xs text-error">{message.safeError}</p> : null}</div><div className="flex shrink-0 gap-1">{capabilities.editQueuedMessage && message.editable ? <Button label="Edit queued message" variant="ghost" size="lg" isIconOnly icon={<Pencil size={15} />} isDisabled={messageBusy} onClick={() => { setEditing(message); setEditDraft(message.content); setError(""); }} /> : null}{capabilities.sendMessage && message.deletable ? <Button label="Delete queued message" variant="ghost" size="lg" isIconOnly icon={<Trash2 size={15} />} isDisabled={messageBusy} onClick={() => setRemoving(message)} /> : null}</div></li>)}</ul> : null}
    {error ? <p className="mb-3 border border-error/30 bg-error/10 px-3 py-2 text-sm text-error" role="alert">{error}</p> : null}
    <form className="flex items-end gap-2" onSubmit={(event) => { event.preventDefault(); void submit(); }}><div className="min-w-0 flex-1"><TextArea label="Message" isLabelHidden value={draft} onChange={setDraft} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} isDisabled={!capabilities.sendMessage || messageBusy} placeholder={capabilities.sendMessage ? "Message the task" : unavailableMessage} rows={2} width="100%" /></div><Button type="submit" label="Send message" size="lg" isIconOnly icon={<Send size={16} />} isDisabled={!capabilities.sendMessage || messageBusy || !draft.trim()} /></form>
    <Dialog isOpen={Boolean(editing)} onOpenChange={(open) => { if (!open && !messageBusy) setEditing(undefined); }} purpose="form" width="min(34rem, calc(100vw - 2rem))" padding={0} aria-label="Edit queued message"><form onSubmit={(event) => { event.preventDefault(); void saveEdit(); }}><DialogHeader title="Edit queued message" subtitle="Only messages the server still accepts can be changed." onOpenChange={(open) => { if (!open && !messageBusy) setEditing(undefined); }} hasDivider />{error ? <p className="mx-5 mt-4 border border-error/30 bg-error/10 px-3 py-2 text-sm text-error" role="alert">{error}</p> : null}<div className="px-5 py-5"><TextArea label="Queued message" isLabelHidden value={editDraft} onChange={setEditDraft} isDisabled={messageBusy} rows={5} hasAutoFocus width="100%" /></div><footer className="flex flex-col-reverse gap-2 border-t border-subtle px-5 py-4 sm:flex-row sm:justify-end md:px-6"><Button type="button" label="Cancel" variant="ghost" size="lg" isDisabled={messageBusy} onClick={() => setEditing(undefined)} /><Button type="submit" label={saving ? "Saving..." : "Save message"} size="lg" isDisabled={messageBusy || !nextEdit || !editChanged} /></footer></form></Dialog>
