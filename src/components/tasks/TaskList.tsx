...
import { ArrowUpRight, ChevronLeft, ChevronRight, Clock3, FileOutput, ListTree, Search, Wrench, X } from "lucide-react";
import Link from "next/link";
import { useEffect, useMemo, useState, type FormEvent } from "react";
import type { Task, TaskListPage, TaskListQuery, TaskStatus, TaskSummary } from "../../lib/api/client";
import { Button } from "../ui/button";
import { Input } from "../ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../ui/select";
import { formatTaskDate, taskStatusLabel } from "./task-ui";
...
export function TaskList({ page, summaries, basePath, query, pageIndex, onQueryChange, onNext, onPrevious }: { page: TaskListPage; summaries: TaskSummary[]; basePath: string; query: TaskListQuery; pageIndex: number; onQueryChange: (query: TaskListQuery) => void; onNext: () => void; onPrevious: () => void }) {
  const [search, setSearch] = useState(query.search ?? "");
  useEffect(() => setSearch(query.search ?? ""), [query.search]);
  const summaryByTaskId = useMemo(() => new Map(summaries.map((summary) => [summary.taskId, summary])), [summaries]);
  const followUpCount = useMemo(() => {
    const counts = new Map<string, number>();
    for (const task of page.items) if (task.sourceTaskId) counts.set(task.sourceTaskId, (counts.get(task.sourceTaskId) ?? 0) + 1);
    return counts;
  }, [page.items]);
  ...
  return <section aria-label="Task list">
    ...
    {page.items.length === 0 ? <div className="grid min-h-52 place-items-center border border-dashed border-border px-6 text-center"><div><Wrench className="mx-auto size-6 text-icon-default" /><h2 className="type-title mt-3 text-foreground">{filtered ? "No tasks match" : "No tasks yet"}</h2><p className="mt-1 text-sm text-secondary">{filtered ? "Try a different search or filter." : "Create a task to run the project in a Botified sandbox."}</p>{filtered ? <Button className="mt-3" variant="quiet" onClick={clearFilters}>Clear filters</Button> : null}</div></div> : <div className="overflow-hidden border border-border"><div className="hidden grid-cols-[minmax(0,1fr)_8rem_11rem] gap-4 border-b border-border bg-surface-low px-4 py-2 font-mono text-[10px] uppercase tracking-[0.1em] text-tertiary md:grid"><span>Task</span><span>Status</span><span className="text-right">Activity</span></div>{page.items.map((task) => <TaskRow key={task.id} task={task} summary={summaryByTaskId.get(task.id)} descendants={followUpCount.get(task.id) ?? 0} basePath={basePath} />)}</div>}
...
function TaskRow({ task, summary, descendants, basePath }: { task: Task; summary: TaskSummary | undefined; descendants: number; basePath: string }) {
  return <Link href={`${basePath}/${task.id}`} className="grid gap-2 border-b border-border px-4 py-3 last:border-b-0 hover:bg-hover md:grid-cols-[minmax(0,1fr)_8rem_11rem] md:items-center md:gap-4"><div className="min-w-0"><p className="truncate text-sm text-foreground">{task.title?.trim() || task.prompt}</p>{task.title ? <p className="mt-1 truncate text-xs text-secondary">{task.prompt}</p> : null}<p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[11px] text-tertiary"><span className="flex items-center gap-1.5"><Clock3 size={12} />{task.id}</span>{task.archivedAt ? <span>Archived</span> : null}{task.sourceTaskId ? <span>Follow-up of {task.sourceTaskId}</span> : null}{descendants ? <span>{descendants} follow-up{descendants === 1 ? "" : "s"}</span> : null}</p></div><div className="flex items-center justify-between gap-3 md:contents"><span className={`w-fit border px-2 py-0.5 font-mono text-[10px] uppercase ${statusClass(task.status)}`}>{terminalLabel(task)}</span><span className="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 text-xs text-secondary"><span className="inline-flex items-center gap-1"><ListTree size={13} />{summary?.eventCount ?? 0} events</span><span className="inline-flex items-center gap-1"><FileOutput size={13} />{summary?.artifactCount ?? 0} artifacts</span><span className="inline-flex items-center gap-1"><span>{formatTaskDate(summary?.updatedAt ?? task.updatedAt)}</span><ArrowUpRight size={15} /></span></span></div></Link>;
}

function terminalLabel(task: Task): string { return task.terminalReason === "not_executed" ? "Not executed" : task.terminalReason === "cleaned_legacy" ? "Cleaned up" : task.terminalReason ? task.terminalReason.replaceAll("_", " ") : taskStatusLabel(task.status); }
