...
function TaskRow({ item, basePath }: { item: TaskListItem; basePath: string }) {
  const { task } = item;
  return <Link href={`${basePath}/${task.id}`} className="grid gap-2 border-b border-border px-4 py-3 last:border-b-0 hover:bg-hover md:grid-cols-[minmax(0,1fr)_11rem_11rem] md:items-center md:gap-4"><div className="min-w-0"><p className="truncate text-sm text-foreground">{task.title?.trim() || task.prompt}</p>{task.title ? <p className="mt-1 truncate text-[13px] text-secondary">{task.prompt}</p> : null}<p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[13px] text-tertiary"><span className="flex items-center gap-1.5"><Clock3 size={13} />{task.id}</span>{item.lifecycle.state === "archived" ? <span>Archived</span> : null}</p></div><div className="flex items-center justify-between gap-3 md:contents"><div className="min-w-0"><span className={`block w-fit border px-2 py-0.5 text-[13px] ${stateClass(item)}`}>{taskProjectionLabel(item)}</span></div><span className="inline-flex items-center justify-end gap-1 text-[13px] text-secondary"><span>{formatTaskDate(task.updatedAt)}</span><ArrowUpRight size={15} /></span></div></Link>;
}
