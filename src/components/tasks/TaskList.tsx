...
export function TaskList({ page, basePath, query, pageIndex, onQueryChange, onNext, onPrevious }: { page: TaskListPage; basePath: string; query: TaskListQuery; pageIndex: number; onQueryChange: (query: TaskListQuery) => void; onNext: () => void; onPrevious: () => void }) {
  ...
  return <section aria-label="Task list">
    <div className="mb-4 flex flex-col gap-2 lg:flex-row lg:items-center">
      <form className="relative min-w-0 flex-1" onSubmit={submitSearch}><label><span className="sr-only">Search tasks</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-tertiary" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9 pr-20" placeholder="Search tasks" /></label>{search ? <Button className="absolute right-10 top-0.5" size="icon" variant="quiet" aria-label="Clear task search" onClick={() => { setSearch(""); onQueryChange(changeTaskQuery(query, {}, ["search", "cursor"])); }}><X size={14} /></Button> : null}<Button className="absolute right-1 top-0.5" size="icon" variant="quiet" aria-label="Apply task search" type="submit"><Search size={14} /></Button></form>
      <Select value={query.archived ?? "exclude"} onValueChange={(value) => onQueryChange(changeTaskQuery(query, { archived:value as NonNullable<TaskListQuery["archived"]> }, ["cursor"]))}><SelectTrigger className="h-9 w-full lg:w-36" aria-label="Task archive"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="exclude">Not archived</SelectItem><SelectItem value="include">All tasks</SelectItem><SelectItem value="only">Archived only</SelectItem></SelectContent></Select>
      <Select value={`${query.sort ?? "updated_at"}:${query.direction ?? "desc"}`} onValueChange={(value) => { const [sort, direction] = value.split(":") as [NonNullable<TaskListQuery["sort"]>, NonNullable<TaskListQuery["direction"]>]; onQueryChange(changeTaskQuery(query, { sort, direction }, ["cursor"])); }}><SelectTrigger className="h-9 w-full lg:w-40" aria-label="Sort tasks"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="updated_at:desc">Most recent</SelectItem><SelectItem value="created_at:asc">Oldest first</SelectItem><SelectItem value="title:asc">Task title</SelectItem></SelectContent></Select>
    </div>
