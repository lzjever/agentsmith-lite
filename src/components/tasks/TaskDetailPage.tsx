...
function TaskDetail({ workspaceId, projectId, taskId, artifactsOnly }: { workspaceId: string; projectId: string; taskId: string; artifactsOnly: boolean }) {
  ...
  const archivedNotice = task.archivedAt ? <div className="flex items-start gap-3 border border-border bg-surface-low px-4 py-3"><Archive className="mt-0.5 size-4 shrink-0 text-icon-default" /><p className="text-sm text-secondary">This task is archived. Its conversation, files, and artifacts remain available.</p></div> : null;
  ...
  return <PageLayout header={header} contentWidth="full">
    ...
    <details className="border-y border-border py-3"><summary className="cursor-pointer text-sm font-medium text-foreground">Task details</summary><div className="mt-4 grid gap-6 lg:grid-cols-[minmax(14rem,.8fr)_minmax(0,1fr)]"><TaskExecutionSummary task={task} filesHref={`/workspaces/${workspaceId}/projects/${projectId}/files?libraryId=${encodeURIComponent(task.fileLibraryId)}`} /><div><h3 className="type-caption text-tertiary">Original prompt</h3><p className="mt-2 whitespace-pre-wrap break-words text-sm text-secondary">{task.prompt}</p></div></div></details>
...
function TaskExecutionSummary({ task, filesHref }: { task: Task; filesHref: string }) {
  return <div><h3 className="type-caption text-tertiary">Execution</h3><dl className="mt-2 grid gap-2 text-sm"><TaskDetailValue label="Library"><Link className="break-all font-mono text-xs text-foreground hover:underline" href={filesHref}>{task.fileLibraryId}</Link></TaskDetailValue><TaskDetailValue label="Mode">{task.executionMode === "live" ? "Live sandbox" : "Dry run"}</TaskDetailValue><TaskDetailValue label="Endpoint"><span className="break-all font-mono text-xs">{task.endpointId}</span></TaskDetailValue><TaskDetailValue label="Run"><span className="break-all font-mono text-xs">{task.runId}</span></TaskDetailValue><TaskDetailValue label="Namespace"><span className="break-all font-mono text-xs">{task.sandbox.namespace}</span></TaskDetailValue>{task.cleanupStatus ? <TaskDetailValue label="Cleanup">{task.cleanupStatus.replaceAll("_", " ").replace(/^./, (value) => value.toUpperCase())}</TaskDetailValue> : null}</dl></div>;
}
