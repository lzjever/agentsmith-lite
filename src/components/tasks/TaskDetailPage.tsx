...
function TaskDetail({ workspaceId, projectId, taskId, artifactsOnly }: { workspaceId: string; projectId: string; taskId: string; artifactsOnly: boolean }) {
  ...
  const sandboxUsable = sandboxState.state === "active" || sandboxState.state === "starting";
  const terminalAvailable = capabilities.openTerminal && sandboxUsable;
  return <PageLayout header={header} contentWidth="full">
    ...
    {archivedNotice}
    {sandboxNotice}
    <div className="flex shrink-0 flex-wrap items-center gap-1 border-b border-border pb-3" role="tablist" aria-label="Task workspace views"><WorkspaceTab active={mode === "conversation"} onClick={() => setMode("conversation")}>Conversation</WorkspaceTab><WorkspaceTab active={mode === "terminal"} disabled={!terminalAvailable && !terminalStarted} onClick={() => { setTerminalStarted(true); setMode("terminal"); }}><TerminalSquare size={14} />Terminal</WorkspaceTab>{showArtifacts ? <WorkspaceTab active={mode === "artifacts"} onClick={() => setMode("artifacts")} className="xl:hidden">Artifacts</WorkspaceTab> : null}</div>
    <div className="grid h-[clamp(24rem,calc(100dvh-20rem),48rem)] min-h-0 min-w-0 gap-4 overflow-hidden md:h-[clamp(24rem,calc(100dvh-12rem),48rem)] xl:grid-cols-[minmax(0,1fr)_18rem]" data-testid="task-workspace">
      ...
      {terminalStarted && sandboxUsable ? <div className={`${mode === "terminal" ? "flex" : "hidden"} min-h-0 min-w-0 flex-1 overflow-hidden`}><TaskTerminalPanel taskId={taskId} active={mode === "terminal"} /></div> : null}
