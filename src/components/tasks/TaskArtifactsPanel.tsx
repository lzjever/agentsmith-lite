...
function ArtifactRow({ taskId, artifact }: { taskId: string; artifact: TaskArtifact }) {
  ...
  return <div className="px-3 py-3">
    <div className="flex items-center gap-3"><FileOutput className="size-4 shrink-0 text-icon-default" /><div className="min-w-0 flex-1"><p className="truncate text-sm text-foreground">{artifact.name}</p><p className="mt-0.5 text-xs text-secondary">{formatArtifactBytes(artifact.bytes)} · {artifact.mediaType ?? "file"} · {formatTaskDate(artifact.createdAt)}</p></div>{safeText ? <Button variant="quiet" size="sm" onClick={() => setTextOpen((open) => !open)}>{textOpen ? "Hide preview" : "Preview"}</Button> : null}{safeImage ? <Button variant="quiet" size="icon" aria-label={`View ${artifact.name}`} title={`View ${artifact.name}`} onClick={() => setImageOpen(true)}><ImageIcon size={16} /></Button> : null}<a className="grid size-9 place-items-center text-secondary hover:bg-hover hover:text-foreground" aria-label={`Download ${artifact.name}`} title={`Download ${artifact.name}`} href={apiClient.artifactDownloadUrl(taskId, artifact.id)}><Download size={16} /></a></div>
