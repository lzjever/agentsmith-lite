...
export function TaskInputsPanel({ taskId, inputs, selectedPaths }: { taskId: string; inputs: TaskInput[]; selectedPaths: string[] }) {
  const selectedPathList = selectedPaths.length ? <div><p className="type-caption text-tertiary">Selected paths</p><ul className="mt-2 space-y-1">{selectedPaths.map((path) => <li key={path} className="break-all font-mono text-xs text-secondary">{path}</li>)}</ul></div> : null;
  if (inputs.length === 0) {
    return <div className="space-y-3">{selectedPathList}<div className="grid min-h-36 place-items-center border border-dashed border-border px-4 text-center"><div><FileInput className="mx-auto size-5 text-icon-default" /><p className="mt-2 text-sm text-secondary">{selectedPaths.length ? "No immutable input snapshot is available." : "No project files were attached."}</p></div></div></div>;
  }
