...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button({ className, variant = "default", size = "default", type = "button", ...props }, ref) {
  const tones: Record<ButtonVariant, string> = {
    default: "border-border bg-surface text-foreground shadow-ambient hover:border-border-input hover:bg-surface-low",
    primary: "border-transparent bg-accent text-white shadow-ambient hover:bg-accent/90 hover:text-white",
    action: "border-border bg-surface-high text-foreground hover:border-border-input hover:bg-hover",
    outline: "border-border bg-transparent text-secondary hover:border-border-input hover:bg-surface-low hover:text-foreground",
    secondary: "border-transparent bg-surface-high text-secondary hover:bg-hover hover:text-foreground",
    ghost: "border-transparent bg-transparent text-secondary hover:bg-hover hover:text-foreground",
    quiet: "border-transparent bg-transparent text-secondary hover:bg-hover hover:text-foreground",
  ...
  return <button ref={ref} type={type} data-visual-prominence={variant === "primary" || variant === "destructive-primary" ? "primary" : undefined} className={cn("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md border text-[13px] font-normal leading-none tracking-[0] transition-[color,background-color,border-color,box-shadow] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/20 focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50", sizes[size], tones[variant], className)} {...props} />;
});
