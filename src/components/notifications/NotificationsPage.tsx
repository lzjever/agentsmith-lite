...
export function NotificationsPage() {
  ...
  async function read(id: string) { setBusyId(id); setMutationError(null); try { const saved = await apiClient.markNotificationRead(id); setItems((current) => current.map((item) => item.id === id ? saved : item)); return true; } catch { setMutationError({ id, action: "read", message: "Notification could not be marked as read." }); return false; } finally { setBusyId(null); } }
  ...
  async function openLinked(event: MouseEvent<HTMLAnchorElement>, item: UserNotification) { event.preventDefault(); if (item.readAt || await read(item.id)) router.push(item.linkPath!); }
...
function notificationReturnPath(): string { const value = new URLSearchParams(window.location.search).get("returnTo"); return value && !value.includes("\\") && value.startsWith("/workspaces/") ? value : "/"; }
