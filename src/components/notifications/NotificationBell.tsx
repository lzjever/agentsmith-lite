...
export function NotificationBell({ returnTo }: { returnTo?: string }) {
  ...
  return <Popover isOpen={open} onOpenChange={onOpenChange} label="Notifications" placement="below" alignment="end" width="min(24rem, calc(100vw - 1rem))" className="p-0" content={<><header className="flex items-center justify-between gap-3 border-b border-subtle px-4 py-3"><strong className="text-sm text-foreground">Notifications</strong>{unread.length > 0 ? <Button label={markingAll ? "Marking..." : "Mark all read"} variant="ghost" size="sm" icon={<CheckCheck size={15} />} isDisabled={actionBusy} onClick={() => void markAllRead()}/> : null}</header><div className="max-h-96 overflow-y-auto">{loading ? <p className="px-4 py-8 text-center text-sm text-secondary">Loading notifications...</p> : null}{error ? <div className="space-y-2 px-4 py-4" role="alert"><p className="text-sm text-error">{error}</p><Button label="Retry" variant="ghost" size="sm" onClick={() => void load()}/></div> : null}{!loading && !error && items.length === 0 ? <p className="px-4 py-8 text-center text-sm text-secondary">No notifications</p> : null}{!loading && !error && items.slice(0, 20).map((item) => <NotificationMenuItem item={item} disabled={actionBusy} onOpen={openItem} onOpenLinked={openLinked} key={item.id} />)}</div><footer className="border-t border-subtle p-2"><Link href={notificationsHref} onClick={() => setOpen(false)} className="block rounded-sm px-3 py-2 text-center text-sm text-secondary hover:bg-hover hover:text-foreground">View all notifications</Link></footer></>}>{({ ref, onClick, ...triggerProps }) => <IconButton ref={ref} label="Open notifications" variant="ghost" className="relative p-2 text-secondary hover:bg-hover hover:text-foreground" onClick={onClick} {...triggerProps} icon={<><Bell size={17} />{unread.length > 0 ? <span className="absolute right-0 top-0 grid min-h-5 min-w-5 place-items-center rounded-full bg-error px-1 text-[13px] leading-5 text-white">{unread.length > 99 ? "99+" : unread.length}</span> : null}</>} />}</Popover>;
}
function NotificationMenuItem({ item, disabled, onOpen, onOpenLinked }: { item: UserNotification; disabled: boolean; onOpen: (item: UserNotification) => Promise<boolean>; onOpenLinked: (event: MouseEvent<HTMLAnchorElement>, item: UserNotification) => Promise<void> }) { const body = <div className={`block px-4 py-3 text-left hover:bg-hover ${item.readAt ? "text-secondary" : "bg-surface-low text-foreground"}`}><p className="text-sm font-medium">{item.title}</p>{item.body ? <p className="mt-1 line-clamp-2 text-xs text-secondary">{item.body}</p> : null}<p className="mt-1 text-xs text-tertiary">{formatLocalDateTime(item.createdAt)}</p></div>; return item.linkPath ? <Link href={item.linkPath} aria-disabled={disabled} tabIndex={disabled ? -1 : undefined} className={disabled ? "pointer-events-none opacity-60" : undefined} onClick={(event) => { if (disabled) event.preventDefault(); else void onOpenLinked(event, item); }}>{body}</Link> : <button type="button" className="block w-full disabled:cursor-not-allowed disabled:opacity-60" disabled={disabled} onClick={() => void onOpen(item)}>{body}</button>; }
