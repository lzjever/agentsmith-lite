...
export function NotificationBell({ returnTo }: { returnTo?: string }) {
  ...
  async function openItem(item: UserNotification) { if (!item.readAt) { try { const saved = await apiClient.markNotificationRead(item.id); setItems((current) => current.map((value) => value.id === saved.id ? saved : value)); } catch { setError("Notification could not be marked as read."); return false; } } setOpen(false); return true; }
