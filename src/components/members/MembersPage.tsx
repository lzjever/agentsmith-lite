...
function ProjectMembersPage({ workspaceId, projectId }: { workspaceId: string; projectId: string }) {
  ...
  return <PageLayout header={<PageHeader title="Members" subtitle="People with access to this project and the role they hold." actions={canAdd ? <Button label="Add member" variant="primary" size="lg" icon={<Plus size={16} />} onClick={openInvite} /> : undefined} />}>
    ...
    <Dialog isOpen={Boolean(selected)} onOpenChange={(open) => !open && setSelected(undefined)} purpose="info" width="min(34rem, calc(100vw - 2rem))" padding={0} aria-label="Member details">{selected ? <><DialogHeader title="Member details" subtitle="Project membership identity." onOpenChange={(open) => !open && setSelected(undefined)} hasDivider padding={5} /><dl className="grid gap-4 px-5 py-5 text-sm sm:grid-cols-[8rem_1fr]"><dt className="text-secondary">Name</dt><dd className="break-all text-foreground">{memberIdentityLabel(selected)}</dd><dt className="text-secondary">Email</dt><dd className="break-all text-foreground">{selected.email}</dd><dt className="text-secondary">Role</dt><dd className="text-foreground">{selected.role}</dd><dt className="text-secondary">Joined</dt><dd className="text-foreground">{new Date(selected.createdAt).toLocaleString("en-US")}</dd><dt className="text-secondary">Updated</dt><dd className="text-foreground">{new Date(selected.updatedAt).toLocaleString("en-US")}</dd></dl></> : null}</Dialog>
  </PageLayout>;
}
