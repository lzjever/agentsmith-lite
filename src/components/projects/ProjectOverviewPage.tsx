...
export function ProjectOverviewPage({ workspaceId, projectId }: { workspaceId: string; projectId: string }) {
  ...
  const [lifecycleState, setLifecycleState] = useState<"loading" | "ready" | "error">("loading");
  const [lifecycleError, setLifecycleError] = useState("");
  const loadLifecycle = useCallback(async () => {
    setLifecycleState("loading");
    setProject(undefined);
    try {
      const settings = await apiClient.projectSettings(projectId);
      setProject(settings.project);
      setLifecycleError("");
      setLifecycleState("ready");
    } catch (reason) {
      setLifecycleError(reason instanceof ApiError ? reason.message : "Project lifecycle could not be loaded.");
  ...
  const lifecycleStatus = project ? project.lifecycleStatus ?? "active" : undefined;
