...
function ProjectOverviewProjectPage({ workspaceId, projectId }: { workspaceId: string; projectId: string }) {
  ...
  return <PageLayout header={header}>
    ...
    {state === "ready" && overview ? <div className="space-y-7">
      ...
      {secondarySteps.length ? <section className="border-t border-subtle pt-6"><p className="type-caption text-tertiary">Also available</p><div className="mt-3 grid gap-px overflow-hidden rounded-md border border-subtle bg-subtle sm:grid-cols-2">{secondarySteps.map((step) => <Link key={step.href} href={`${base}/${step.href}`} className="group flex min-h-28 items-start justify-between gap-4 bg-surface p-4 text-foreground no-underline hover:bg-hover"><span><step.icon size={17} className="mb-3 text-accent" /><strong className="block text-sm font-medium">{step.label}</strong><span className="mt-1 block text-sm text-secondary">{step.description}</span></span><ArrowRight size={16} className="mt-1 shrink-0 text-icon-default transition-transform group-hover:translate-x-0.5" /></Link>)}</div></section> : null}
    </div> : null}
  </PageLayout>;
}
