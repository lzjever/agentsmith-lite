...
function WorkspaceMembers({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout header={<PageHeader title="Workspace members" subtitle="People with access to this workspace." actions={canManage ? <Button label="Add member" icon={<Plus size={16} />} variant="primary" size="lg" isDisabled={mutationBusy} onClick={() => { setMutationError(undefined); setOpen(true); }} /> : undefined} />}>
    ...
    <Dialog isOpen={Boolean(selected)} onOpenChange={(next) => !next && setSelected(undefined)} purpose="info" width="min(34rem, calc(100vw - 2rem))" padding={0} aria-label="Member details">{selected ? <><DialogHeader title="Member details" subtitle="Workspace membership identity." onOpenChange={(next) => !next && setSelected(undefined)} hasDivider /><dl className="grid gap-4 px-5 py-5 text-sm sm:grid-cols-[8rem_1fr]"><dt className="text-secondary">Name</dt><dd className="break-all text-foreground">{memberLabel(selected)}</dd><dt className="text-secondary">Email</dt><dd className="break-all text-foreground">{selected.email}</dd><dt className="text-secondary">Role</dt><dd>{selected.role}</dd><dt className="text-secondary">Joined</dt><dd>{formatLocalDateTime(selected.createdAt)}</dd></dl></> : null}</Dialog>
