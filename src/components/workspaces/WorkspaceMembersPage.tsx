...
function WorkspaceMembers({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout header={<PageHeader title="Workspace members" subtitle="People with access to this workspace." actions={canManage ? <Button label="Add member" icon={<Plus size={16} />} variant="primary" size="lg" isDisabled={mutationBusy} onClick={() => { setMutationError(undefined); setOpen(true); }} /> : undefined} />}>
    ...
    <Dialog isOpen={open} onOpenChange={(next) => { if (!mutationBusy) setOpen(next); }} purpose="info" width="min(34rem, calc(100vw - 2rem))" padding={0} aria-label="Add workspace member"><form onSubmit={add}><DialogHeader title="Add workspace member" subtitle="Grant an existing identity access to this workspace." onOpenChange={(next) => { if (!mutationBusy) setOpen(next); }} hasDivider padding={5} />{mutationError ? <div className="px-5 pt-4"><MutationNotice error={mutationError} onDismiss={() => setMutationError(undefined)} /></div> : null}<div className="grid gap-4 px-5 py-5"><label className="grid gap-2 text-sm text-primary">Email<Input value={email} onChange={(event) => setEmail(event.target.value)} type="email" disabled={mutationBusy} /></label><Selector label="Workspace member role" isLabelHidden options={[{ value: "admin", label: "Admin" }, { value: "member", label: "Member" }, { value: "viewer", label: "Viewer" }]} value={role} onChange={(value) => setRole(value as Exclude<WorkspaceMemberRole, "owner">)} isDisabled={mutationBusy} size="lg" /></div><footer className="flex flex-col-reverse gap-2 border-t border-subtle px-5 py-4 sm:flex-row sm:justify-end md:px-6"><Button label="Cancel" type="button" variant="ghost" size="lg" onClick={() => setOpen(false)} isDisabled={mutationBusy} /><Button label={busyUserId === "new" ? "Adding..." : "Add member"} type="submit" variant="primary" size="lg" isDisabled={!email.trim() || !canManage || mutationBusy} /></footer></form></Dialog>
    <Dialog isOpen={Boolean(selected)} onOpenChange={(next) => !next && setSelected(undefined)} purpose="info" width="min(34rem, calc(100vw - 2rem))" padding={0} aria-label="Member details">{selected ? <><DialogHeader title="Member details" subtitle="Workspace membership identity." onOpenChange={(next) => !next && setSelected(undefined)} hasDivider padding={5} /><dl className="grid gap-4 px-5 py-5 text-sm sm:grid-cols-[8rem_1fr]"><dt className="text-secondary">Name</dt><dd className="break-all text-foreground">{memberLabel(selected)}</dd><dt className="text-secondary">Email</dt><dd className="break-all text-foreground">{selected.email}</dd><dt className="text-secondary">Role</dt><dd>{selected.role}</dd><dt className="text-secondary">Joined</dt><dd>{new Date(selected.createdAt).toLocaleString("en-US")}</dd></dl></> : null}</Dialog>
