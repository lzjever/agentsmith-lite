...
import { Badge, Banner, Button, DialogHeader, EmptyState, IconButton, Layout, LayoutContent, Selector, Spinner, Text, TextInput } from "@astryxdesign/core";
...
function WorkspaceMembers({ workspaceId }: { workspaceId: string }) {
  ...
  return <PageLayout header={<PageHeader title="Workspace members" subtitle="People with access to this workspace." actions={canManage ? <Button label="Add member" icon={<Plus size={16} />} variant="primary" size="lg" isDisabled={mutationBusy} onClick={() => { setMutationError(undefined); setOpen(true); }} /> : undefined} />}>
    ...
    <Dialog isOpen={open} onOpenChange={handleMemberOpenChange} purpose="form" width="min(34rem, calc(100vw - 2rem))" aria-label="Add workspace member"><Layout defaultHasDividers header={<DialogHeader title="Add workspace member" subtitle="Grant an existing identity access to this workspace." onOpenChange={handleMemberOpenChange} />} content={<LayoutContent><form id="workspace-member-add-form" onSubmit={add}><div className="grid gap-4">{mutationError ? <MutationNotice error={mutationError} onDismiss={() => setMutationError(undefined)} /> : null}<TextInput label="Email" value={email} onChange={setEmail} type="email" isDisabled={mutationBusy} width="100%" /><Selector label="Workspace member role" options={[{ value: "admin", label: "Admin" }, { value: "member", label: "Member" }, { value: "viewer", label: "Viewer" }]} value={role} onChange={(value) => setRole(value as Exclude<WorkspaceMemberRole, "owner">)} isDisabled={mutationBusy} size="lg" /></div></form></LayoutContent>} footer={<DialogFooter secondaryAction={<Button label="Cancel" type="button" variant="ghost" size="lg" onClick={() => handleMemberOpenChange(false)} isDisabled={mutationBusy} />} primaryAction={<Button label={busyUserId === "new" ? "Adding..." : "Add member"} type="submit" form="workspace-member-add-form" variant="primary" size="lg" isDisabled={!email.trim() || !canManage || mutationBusy} isLoading={busyUserId === "new"} />} />} /></Dialog>
    <Dialog isOpen={Boolean(selected)} onOpenChange={(next) => !next && setSelected(undefined)} purpose="info" width="min(34rem, calc(100vw - 2rem))" aria-label="Member details">{selected ? <Layout defaultHasDividers header={<DialogHeader title="Member details" subtitle="Workspace membership identity." onOpenChange={(next) => !next && setSelected(undefined)} />} content={<LayoutContent><dl className="grid gap-4 sm:grid-cols-[8rem_1fr]"><dt><Text color="secondary">Name</Text></dt><dd><Text wordBreak="break-all">{memberLabel(selected)}</Text></dd><dt><Text color="secondary">Email</Text></dt><dd><Text wordBreak="break-all">{selected.email}</Text></dd><dt><Text color="secondary">Role</Text></dt><dd><Text>{selected.role}</Text></dd><dt><Text color="secondary">Joined</Text></dt><dd><Text>{formatLocalDateTime(selected.createdAt)}</Text></dd></dl></LayoutContent>} /> : null}</Dialog>
    <Dialog isOpen={Boolean(memberToRemove)} onOpenChange={(next) => { if (mutationBusy) return; if (!next) { setMemberToRemove(undefined); setMutationError(undefined); } }} purpose="form" role="alertdialog" width="min(32rem, calc(100vw - 2rem))" aria-labelledby="workspace-member-remove-title" aria-describedby="workspace-member-remove-description"><Layout defaultHasDividers header={<DialogHeader id="workspace-member-remove-title" title="Remove workspace member" />} content={<LayoutContent><div className="grid gap-4"><Text id="workspace-member-remove-description" as="p" display="block" color="secondary">{memberToRemove ? `Remove ${memberLabel(memberToRemove)} from this workspace? They will lose access to its projects.` : "This member is no longer available."}</Text>{mutationError ? <MutationNotice error={mutationError} onDismiss={() => setMutationError(undefined)} /> : null}</div></LayoutContent>} footer={<DialogFooter secondaryAction={<Button label="Cancel" variant="ghost" size="lg" isDisabled={mutationBusy} onClick={() => setMemberToRemove(undefined)} />} primaryAction={<Button label="Remove member" variant="destructive" size="lg" isDisabled={!memberToRemove || mutationBusy} isLoading={Boolean(memberToRemove && busyUserId === memberToRemove.userId)} onClick={() => { if (memberToRemove) void remove(memberToRemove); }} />} />} /></Dialog>
  </PageLayout>;
}
