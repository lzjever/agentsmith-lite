...
export function WorkspaceOverviewPage({ workspaceId }: { workspaceId: string }) {
  ...
  function created(project: Project) { setWorkspace((current) => current ? { ...current, projects: [...current.projects, project] } : current); setCreateOpen(false); }
  const canCreate = workspace?.capabilities.canCreateProject === true;
  const canManage = workspace?.capabilities.canManageMembers === true;
  const canOpenSettings = canManage || workspace?.memberRole === "owner";
