...
import { apiClient, type Endpoint, type ProjectAlertRule } from "../../lib/api/client";
...
export function AlertRulesPanel({ projectId, canManage }: { projectId: string; canManage: boolean }) {
  ...
  async function toggle(rule: ProjectAlertRule) {
    ...
    try {
      ...
      toast.error("Alert rule could not be updated.");
    } finally {
      setBusyRuleId(null);
    }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      toast.error("Alert rule could not be deleted.");
      throw error;
    }
  }
