...
export function ShellNavigation({ workspace, project, pathname, collapsed = false, onNavigate }: NavigationProps) {
  ...
  return <nav aria-label="Primary navigation" className="flex-1 overflow-y-auto px-2 py-4">{groups.map((group) => <section className="mb-5" key={group.label}><p className={cn("mb-1 px-3 font-mono text-[10px] uppercase tracking-[0.12em] text-tertiary", collapsed && "sr-only")}>{group.label}</p><div className="space-y-1">{group.items.map((item) => <NavigationLink key={item.href} item={item} pathname={pathname} collapsed={collapsed} onNavigate={onNavigate} />)}</div></section>)}</nav>;
}
...
function NavigationLink({ item, pathname, collapsed, onNavigate }: { item: NavItem; pathname: string; collapsed: boolean; onNavigate?: (() => void) | undefined }) {
  ...
  const link = <Link href={item.href} {...(onNavigate ? { onClick: onNavigate } : {})} aria-label={collapsed ? item.label : undefined} aria-current={active ? "page" : undefined} className={cn("flex h-9 items-center rounded-sm border border-transparent text-sm transition-[background-color,border-color,color] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40", collapsed ? "justify-center px-0" : "gap-3 px-3", active ? "font-medium text-foreground" : "text-secondary hover:bg-surface-low/18 hover:text-foreground")}><item.icon className={cn("size-5 shrink-0", active ? "text-foreground" : "text-icon-default")} /><span className={cn("truncate", collapsed && "hidden")}>{item.label}</span></Link>;
