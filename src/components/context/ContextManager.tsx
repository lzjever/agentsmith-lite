...
function ContextRouteManager({ workspaceId, projectId }: { workspaceId: string; projectId?: string }) {
  ...
  const dirty = result?.canWrite === true && (selected
    ...
    : contextKey.length > 0 || content.length > 0 || contentType !== "text");
  useEffect(() => {
