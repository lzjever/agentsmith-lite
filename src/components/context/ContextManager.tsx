...
function ContextRouteManager({ workspaceId, projectId }: { workspaceId: string; projectId?: string }) {
  ...
  return <PageLayout contentWidth="full" header={<PageHeader title="Context" subtitle={projectId ? "Saved instructions and reference data for this workspace and project." : "Saved instructions and reference data for this workspace."} />}>
    <Tabs value={scope} onValueChange={(value) => navigate({ kind: "scope", scope: value as ContextScope })}><TabsList aria-label="Context scope" className="mb-5 flex h-auto flex-wrap justify-start">{tabs.map((tab) => <TabsTrigger key={tab.scope} value={tab.scope} disabled={saving || deleting} onClick={() => navigate({ kind: "scope", scope: tab.scope })}>{tab.label}</TabsTrigger>)}</TabsList>{tabs.map((tab) => <TabsContent key={tab.scope} value={tab.scope}><p className="mb-5 text-sm text-secondary">{tab.description}</p>{state === "loading" ? <PageState>Loading context...</PageState> : null}{state === "error" ? <PageState><div className="space-y-3"><p role="alert" className="text-error">{error}</p><Button label="Try again" size="lg" onClick={() => void load()} /></div></PageState> : null}{state === "ready" && result ? <div className="grid gap-6 lg:grid-cols-[17rem_minmax(0,1fr)]"><section className="border-y border-border py-3 lg:border-y-0 lg:border-r lg:pr-5"><div className="mb-3 flex items-center justify-between"><h2 className="type-title">Entries</h2>{result.canWrite ? <IconButton label="New context entry" size="lg" variant="ghost" icon={<FilePlus2 size={17} />} isDisabled={saving || deleting} onClick={() => navigate({ kind: "new" })} /> : null}</div>{result.items.length === 0 ? <p className="py-4 text-sm text-secondary">No context entries yet.</p> : <div className="space-y-1">{result.items.map((entry) => <button key={entry.id} type="button" disabled={saving || deleting} onClick={() => navigate({ kind: "entry", contextKey: entry.contextKey })} className={`w-full px-3 py-2 text-left text-sm disabled:cursor-not-allowed disabled:opacity-60 ${selected?.id === entry.id ? "bg-surface text-foreground" : "text-secondary hover:bg-hover hover:text-foreground"}`}><span className="block truncate">{entry.contextKey}</span><span className="mt-1 block font-mono text-[10px] uppercase text-tertiary">{entry.contentType}</span></button>)}</div>}</section><section className="min-w-0"><div className="mb-4"><h2 className="type-title">{selected ? "Edit entry" : "New entry"}</h2>{!result.canWrite ? <p className="mt-1 text-sm text-secondary">Your access to this context is read-only.</p> : null}</div><div className="grid gap-4"><label className="grid gap-1.5 text-sm text-secondary">Key<Input value={contextKey} disabled={!result.canWrite || saving || deleting} onChange={(event) => setContextKey(event.target.value)} placeholder="for example, project.conventions" /></label><Selector label="Content type" options={contentTypes.map((type) => ({ value: type, label: type }))} value={contentType} onChange={(value) => setContentType(value as ContextContentType)} isDisabled={!result.canWrite || saving || deleting} size="lg" /><label className="grid gap-1.5 text-sm text-secondary">Content<Textarea value={content} disabled={!result.canWrite || saving || deleting} onChange={(event) => setContent(event.target.value)} className="min-h-64 font-mono" /></label>{error ? <div className="flex flex-wrap items-center gap-3 text-sm text-error" role="alert"><span>{error}</span>{conflict ? <Button label="Reload latest" size="md" variant="secondary" onClick={() => void load(false, selected?.id)} /> : null}</div> : null}<div className="flex flex-wrap gap-2">{result.canWrite ? <Button label={saving ? "Saving..." : "Save"} size="lg" variant="primary" isDisabled={!dirty || saving || deleting} onClick={() => void save()} /> : null}{result.canWrite && selected ? <Button label="Delete" size="lg" variant="destructive" icon={<Trash2 size={16} />} isDisabled={saving || deleting} onClick={() => setDeleteOpen(true)} /> : null}</div></div></section></div> : null}</TabsContent>)}</Tabs>
