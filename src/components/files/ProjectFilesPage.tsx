...
function ProjectFiles({ workspaceId, projectId }: { workspaceId: string | undefined; projectId: string }) {
  ...
  return <PageLayout contentWidth="full" header={<PageHeader title="Files" subtitle="Project files available to this project and its tasks." actions={<div className="flex items-center gap-2"><Button variant="quiet" size="icon" title="Refresh files" aria-label="Refresh files" onClick={() => void load()} disabled={state === "loading" || mutationBusy}><RefreshCw size={16} /></Button>{canWrite ? <><Button onClick={() => input.current?.click()} disabled={mutationBusy}>{uploading ? <RefreshCw className="animate-spin" size={16} /> : <Upload size={16} />}{uploading ? "Uploading" : "Upload"}</Button><input ref={input} hidden type="file" onChange={selectUpload} /></> : null}</div>} />}>
    <div className="grid min-h-[34rem] gap-4 lg:grid-cols-[minmax(0,1fr)_19rem]">
      <section className={`min-w-0 overflow-hidden rounded-md border bg-surface transition-colors ${dropReady ? "border-accent ring-2 ring-accent/25" : "border-subtle"}`} aria-label="Project files" aria-busy={state === "loading"} onDragEnter={(event)=>{event.preventDefault();if(canWrite&&!mutationBusy)setDropReady(true);}} onDragOver={(event)=>event.preventDefault()} onDragLeave={(event)=>{if(canWrite&&!event.currentTarget.contains(event.relatedTarget as Node))setDropReady(false);}} onDrop={(event)=>{event.preventDefault();setDropReady(false);if(!canWrite||mutationBusy)return;const file=event.dataTransfer.files?.[0];if(file)void upload(file);}}>
