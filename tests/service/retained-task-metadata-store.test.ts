...
describe("retained chat, task, and artifact store metadata", () => {
  ...
  it("persists queued messages and computes artifact-only task summaries", async () => {
    ...
    await store.createTaskMessage({ id: "message_1", taskId: "task_1", content: "continue", targetTaskId: "task_2", deliveryStatus:"successor_created", createdAt: "2026-07-01T00:00:03.000Z" });

    assert.deepEqual(await store.findTaskSummary("task_1"), { taskId: "task_1", artifactCount: 1, updatedAt: "2026-07-01T00:00:00.000Z" });
    assert.deepEqual((await store.listTaskMessages("task_1")).map((value) => [value.content, value.targetTaskId]), [["continue", "task_2"]]);
  });
});
