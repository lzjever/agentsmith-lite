...
describe("Phase 3 durable Botified task sessions", () => {
  ...
  it("fails closed when the live Sandbox run record is missing",async()=>{
    ...
    await assert.rejects(()=>setup.services.tasks.sendTaskMessage(setup.userId,task.id,"cannot start cold","cold"),/cold start is not available/i);
  ...
  it("retains ownership and denies delivery, Terminal, archive, and delete until cleanup is confirmed",async()=>{
    ...
    await assert.rejects(()=>setup.services.tasks.sendTaskMessage(setup.userId,task.id,"blocked","blocked-message"),/sandbox is released/i);await assert.rejects(()=>setup.services.tasks.openTaskTerminal(setup.userId,task.id),/sandbox is running/i);await assert.rejects(()=>setup.services.tasks.authorizeBotifiedChatCompletion(task.id,task.runId,"service-key"),/not active/i);await assert.rejects(()=>setup.services.tasks.archiveTask(setup.userId,task.id,"blocked-archive"),/release the task sandbox/i);await assert.rejects(()=>setup.services.tasks.deleteTask(setup.userId,task.id,"blocked-delete"),/release the task sandbox/i);
    ...
    const cleaning=await setup.store.sandboxRuns.get(task.runId);assert.ok(cleaning);await setup.store.sandboxRuns.updateWithFencing(cleaning.runId,cleaning.fencingToken,{...cleaning,phase:"cleaned",cleanupStatus:"cleaned",fencingToken:cleaning.fencingToken+1,updatedAt:"2026-07-19T00:11:00.000Z"});
