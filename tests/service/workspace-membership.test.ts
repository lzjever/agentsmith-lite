...
describe("workspace memberships", () => {
  it("uses explicit memberships for access and protects the final owner", async () => {
    ...
    const ownerWorkspace = await services.workspaces.workspaceDetail(owner.user.id,workspace.id);
    ...
    assert.deepEqual((await services.workspaceMemberships.list(owner.user.id,workspace.id)).items.find((entry)=>entry.userId===owner.user.id)?.displayName,"Owner display");
    const memberWorkspace = await services.workspaces.workspaceDetail(member.user.id,workspace.id);
    assert.equal(memberWorkspace.workspace.id,workspace.id);
    ...
    assert.equal((await services.workspaces.workspaceDetail(viewer.user.id,workspace.id)).workspace.id,workspace.id);
    ...
    const ownerMembership=(await services.workspaceMemberships.list(owner.user.id,workspace.id)).items.find((entry)=>entry.userId===owner.user.id)!;
    ...
    const adminMembership=(await services.workspaceMemberships.list(owner.user.id,workspace.id)).items.find((entry)=>entry.userId===admin.user.id)!;
