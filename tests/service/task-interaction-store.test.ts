...
describe("task interaction store", () => {
  ...
  it("rolls terminal lifecycle, interaction, artifact, and source cursor updates back together", async () => {
    const store = createLocalInMemoryProductStore();
    await store.createProject(project());
    await store.createTask(task());
    ...
    await assert.rejects(store.persistTaskInteractionMutation({
      ...
      lifecycle:{kind:"terminal",terminalReason:"completed",updatedAt:timestamp(3),auditEvent:{id:"terminal-audit",projectId:"project",actorId:null,action:"task.completed",status:"accepted",resourceKind:"task",resourceId:"task_interactions",createdAt:timestamp(3)},successors:[]},
      sourceSync:{expectedSourceCursor:null,sourceCursor:"terminal-cursor",historyStatus:"complete",lastSyncedAt:timestamp(3)}
    }), /lifecycle conflict/);

    assert.equal((await store.findTask("task_interactions"))?.terminalReason ?? null, null);
    assert.deepEqual(await store.listTaskArtifacts("task_interactions"), []);
    assert.deepEqual(await store.listTaskInteractionChanges("task_interactions",0,10), []);
...
function task(): PersistedAgentTask {
  return { id:"task_interactions",workspaceId:"workspace",projectId:"project",endpointId:"endpoint",prompt:"work",status:"running",runId:"run",executionMode:"live",sandbox:{namespace:"agentsmith",resources:[]},createdAt:timestamp(0),updatedAt:timestamp(0) };
}
