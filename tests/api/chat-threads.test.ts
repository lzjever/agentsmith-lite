...
describe("project chat threads API", () => {
  ...
  it("rejects removed chat query and command inputs",async()=>{const thread=await requestJson("POST",`/api/v1/projects/${projectId}/chat/threads`,{endpointId});assert.equal((await request("GET",`/api/v1/projects/${projectId}/chat/threads?runnerId=legacy`)).response.status,400);assert.equal((await request("GET",`/api/v1/projects/${projectId}/chat/threads/${thread.id}/messages?includeAttachments=true`)).response.status,400);assert.equal((await request("DELETE",`/api/v1/projects/${projectId}/chat/threads/${thread.id}`,{force:true})).response.status,400);assert.equal((await request("GET",`/api/v1/projects/${projectId}/chat/threads/${thread.id}/messages`)).response.status,200);});
  ...
  async function request(method: string, pathname: string, body?: unknown, session = cookie, token = csrf, idempotencyKey?: string): Promise<{ response: Response; body: unknown }> {
    const response = await fetch(api.baseUrl + pathname, { method, headers: { ...(body === undefined ? {} : { "content-type": "application/json" }), ...(session ? { cookie: session } : {}), ...(["POST", "PATCH", "DELETE"].includes(method) && token ? { "x-csrf-token": token } : {}), ...((method === "POST" && (pathname === "/api/v1/workspaces" || /^\/api\/v1\/workspaces\/[^/]+\/(projects|members)$/.test(pathname) || /^\/api\/v1\/projects\/[^/]+\/(credentials|endpoints|members)$/.test(pathname) || /^\/api\/v1\/projects\/[^/]+\/chat\/threads$/.test(pathname) || /^\/api\/v1\/projects\/[^/]+\/chat\/threads\/[^/]+\/messages\/[^/]+\/branch$/.test(pathname))) || (["PATCH", "DELETE"].includes(method) && /^\/api\/v1\/projects\/[^/]+\/chat\/threads\/[^/]+(?:\/messages\/[^/]+)?$/.test(pathname)) ? { "idempotency-key": idempotencyKey ?? crypto.randomUUID() } : {}) }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
