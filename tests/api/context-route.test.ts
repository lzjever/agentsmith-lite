...
describe("context API", () => {
  ...
  async function request(method: string, pathname: string, body?: unknown, session = cookie, token = csrf): Promise<{ response: Response; body: any }> {
    const response = await fetch(api.baseUrl + pathname, { method, headers: { ...(body === undefined ? {} : { "content-type": "application/json" }), ...(session ? { cookie: session } : {}), ...(["POST", "PUT", "PATCH", "DELETE"].includes(method) && token ? { "x-csrf-token": token } : {}) }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
