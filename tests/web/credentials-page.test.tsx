...
describe("CredentialsPage",()=>{
  ...
  it("becomes read-only when credential management permission is revoked during a mutation",async()=>{
    ...
    let capabilityReads=0;apiClient.credentials=async()=>[credential];apiClient.projectCapabilities=async()=>capabilityReads++===0?manager:viewer;apiClient.createCredential=async()=>{throw new ApiError(403,"Credential management permission was revoked.");};
    try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"New credential"}));const dialog=await screen.findByRole("dialog",{name:"New credential"});fireEvent.change(within(dialog).getByRole("textbox",{name:"Name"}),{target:{value:"New key"}});fireEvent.change(within(dialog).getByRole("textbox",{name:"Base URL"}),{target:{value:"https://api.example.test/v1"}});fireEvent.change(within(dialog).getByLabelText("API key"),{target:{value:"secret-value"}});await act(async()=>{fireEvent.click(within(dialog).getByRole("button",{name:"Create credential"}));await new Promise(resolve=>setTimeout(resolve,0));});assert.equal(screen.queryByRole("dialog",{name:"New credential"}),null);assert.equal(screen.queryByRole("button",{name:"New credential"}),null);assert.equal(screen.queryByRole("button",{name:/Rotate/}),null);assert.ok(screen.getByText("DeepSeek"));assert.ok(capabilityReads>=2);}finally{Object.assign(apiClient,original);}
  });
  it("clears credential metadata when a mutation discovers project access was removed",async()=>{
    const original={credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities,createCredential:apiClient.createCredential};let removed=false;
    apiClient.credentials=async()=>{if(removed)throw new ApiError(403,"Project access denied");return[credential];};apiClient.projectCapabilities=async()=>manager;apiClient.createCredential=async()=>{removed=true;throw new ApiError(403,"Project access denied");};
    try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"New credential"}));const dialog=await screen.findByRole("dialog",{name:"New credential"});fireEvent.change(within(dialog).getByRole("textbox",{name:"Name"}),{target:{value:"New key"}});fireEvent.change(within(dialog).getByRole("textbox",{name:"Base URL"}),{target:{value:"https://api.example.test/v1"}});fireEvent.change(within(dialog).getByLabelText("API key"),{target:{value:"secret-value"}});fireEvent.click(within(dialog).getByRole("button",{name:"Create credential"}));await screen.findByRole("heading",{name:"Credentials unavailable"});assert.equal(screen.queryByText("DeepSeek"),null);assert.equal(screen.queryByRole("dialog",{name:"New credential"}),null);}finally{Object.assign(apiClient,original);}
  });
  it("becomes read-only when the project is archived during a credential mutation",async()=>{
    const original={credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities,rotateCredential:apiClient.rotateCredential};
    apiClient.credentials=async()=>[credential];apiClient.projectCapabilities=async()=>manager;apiClient.rotateCredential=async()=>{throw new ApiError(409,"Project is archived");};
    try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"Rotate"}));const dialog=await screen.findByRole("dialog",{name:"Rotate DeepSeek"});fireEvent.change(within(dialog).getByLabelText("API key"),{target:{value:"new-key"}});await act(async()=>{fireEvent.click(within(dialog).getByRole("button",{name:"Rotate credential"}));await new Promise(resolve=>setTimeout(resolve,0));});assert.equal(screen.queryByRole("dialog",{name:"Rotate DeepSeek"}),null);assert.equal(screen.queryByRole("button",{name:"New credential"}),null);assert.equal(screen.queryByRole("button",{name:"Rotate"}),null);assert.match(screen.getByRole("alert").textContent??"",/now read-only/i);}finally{Object.assign(apiClient,original);}
  });
  ...
  it("removes a credential that disappeared before deletion completed",async()=>{
    const original={credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities,deleteCredential:apiClient.deleteCredential};let missing=false;let reads=0;
    apiClient.credentials=async()=>{reads+=1;return missing?[]:[credential];};apiClient.projectCapabilities=async()=>manager;apiClient.deleteCredential=async()=>{missing=true;throw new ApiError(404,"Credential not found");};
    try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"Delete DeepSeek"}));const dialog=await screen.findByRole("alertdialog",{name:"Delete DeepSeek"});fireEvent.click(within(dialog).getByRole("button",{name:"Delete credential"}));await waitFor(()=>assert.equal(reads,2));await waitFor(()=>assert.equal(screen.queryByRole("alertdialog",{name:"Delete DeepSeek"}),null));assert.equal(screen.queryByText("DeepSeek"),null);}finally{Object.assign(apiClient,original);}
  });
  it("retries credential deletion with the refreshed version",async()=>{
    const original={credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities,deleteCredential:apiClient.deleteCredential};let current=credential;let reads=0;const versions:number[]=[];
    apiClient.credentials=async()=>{reads+=1;return[current];};apiClient.projectCapabilities=async()=>manager;apiClient.deleteCredential=async(_projectId,_credentialId,version)=>{versions.push(version);if(versions.length===1){current={...current,version:2,updatedAt:"2026-01-02T00:00:00.000Z"};throw new ApiError(409,"Credential changed elsewhere. Refresh and try again.");}return{deleted:true};};
    try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"Delete DeepSeek"}));const dialog=await screen.findByRole("alertdialog",{name:"Delete DeepSeek"});fireEvent.click(within(dialog).getByRole("button",{name:"Delete credential"}));await waitFor(()=>assert.equal(reads,2));await screen.findByText("Version 2");fireEvent.click(within(dialog).getByRole("button",{name:"Delete credential"}));await waitFor(()=>assert.deepEqual(versions,[1,2]));}finally{Object.assign(apiClient,original);}
  });
  it("reuses a credential deletion key after an unknown network result",async()=>{const original={credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities,deleteCredential:apiClient.deleteCredential};const keys:string[]=[];let attempts=0;apiClient.credentials=async()=>[credential];apiClient.projectCapabilities=async()=>manager;apiClient.deleteCredential=(async(_project:string,_credential:string,_version:number,key:string)=>{keys.push(key);if(++attempts===1)throw new Error("connection closed");return{deleted:true};}) as typeof apiClient.deleteCredential;try{render(<CredentialsPage projectId="project_1"/>);fireEvent.click(await screen.findByRole("button",{name:"Delete DeepSeek"}));const dialog=await screen.findByRole("alertdialog",{name:"Delete DeepSeek"});fireEvent.click(within(dialog).getByRole("button",{name:"Delete credential"}));await waitFor(()=>assert.equal(attempts,1));fireEvent.click(within(dialog).getByRole("button",{name:"Delete credential"}));await waitFor(()=>assert.equal(attempts,2));assert.ok(keys[0]);assert.equal(keys[1],keys[0]);await waitFor(()=>assert.equal(screen.queryByText("DeepSeek"),null));}finally{Object.assign(apiClient,original);}});
});
