...
const capabilities: ProjectCapabilities = { canManageEndpoints: false, canManageMembers: false, canManagePolicy: true, canWriteFiles: true, canCreateTasks: true };
...
describe("project resource pages", () => {
  ...
  it("filters audit events, opens a safe detail view, and never renders unsupported sensitive event fields", async () => {
    ...
    try {
      ...
      assert.ok(await screen.findByRole("option", { name: "Sent task message" }));
  ...
  it("keeps linkable audit filters in the URL and visibly labels the time range", async () => {
    ...
    try {
      ...
      fireEvent.click(screen.getByRole("combobox", { name: "Action" }));
      fireEvent.click(await screen.findByRole("option", { name: "Sent task message" }));
      assert.equal(new URL(window.location.href).searchParams.get("action"), "task.message.create");
      await screen.findByText("No audit events match this query.");
