...
import { ApiError, apiClient, type ProjectAlert, type ProjectAuditEvent, type ProjectCapabilities, type ProjectPolicyInput, type ProjectResourcePolicy, type ProjectResourceUsage, type ProjectUsageOverview } from "../../src/lib/api/client.js";
...
describe("project resource pages", () => {
  ...
  it("filters audit events, opens a safe detail view, and never renders unsupported sensitive event fields", async () => {
    ...
    apiClient.audit = async () => ({ items: [event], nextCursor: null });
    try {
  ...
  it("uses project alert history for the Notifications tab and only allows projected managers to resolve it", async () => {
    const original = snapshotClient();
    const alert: ProjectAlert = { id: "history_1", projectId, type: "task_failure", status: "active", deliveryStatus: "delivered", createdAt: policy.createdAt, updatedAt: policy.updatedAt, resolvedAt: null, dismissedAt: null };
    ...
    } finally { restoreClient(original); apiClient.transitionAlert = original.transitionAlert; }
  });
...
function snapshotClient() { return { policy: apiClient.policy, updatePolicy: apiClient.updatePolicy, usage: apiClient.usage, alerts: apiClient.alerts, audit: apiClient.audit, projectCapabilities: apiClient.projectCapabilities, transitionAlert: apiClient.transitionAlert, alertRules: apiClient.alertRules }; }
function restoreClient(original: ReturnType<typeof snapshotClient>) { apiClient.policy = original.policy; apiClient.updatePolicy = original.updatePolicy; apiClient.usage = original.usage; apiClient.alerts = original.alerts; apiClient.audit = original.audit; apiClient.projectCapabilities = original.projectCapabilities; apiClient.transitionAlert = original.transitionAlert; apiClient.alertRules = original.alertRules; }
function installDom() {
