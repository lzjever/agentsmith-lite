...
describe("project files API client", () => {
  it("uses the project API for list, binary upload, download, and delete", async () => {
    ...
    const requests: Array<{ url: string; method: string; contentType: string | null; csrf: string | null; body: BodyInit | null | undefined }> = [];
    globalThis.fetch = async (input, init) => {
      const rawUrl = typeof input === "string" ? input : input instanceof URL ? input.toString() : input.url;
      const request = new Request(new URL(rawUrl, "http://localhost"), init);
      requests.push({ url: request.url, method: request.method, contentType: request.headers.get("content-type"), csrf: request.headers.get("x-csrf-token"), body: init?.body });
    ...
    try {
      ...
      await apiClient.uploadFile("project_1", "files/reports/brief.bin", new Blob([Uint8Array.from([0, 255, 7])], { type: "application/octet-stream" }) as File);
      ...
      assert.deepEqual(requests[2] && { method: requests[2].method, contentType: requests[2].contentType, csrf: requests[2].csrf }, { method: "PUT", contentType: "application/octet-stream", csrf: "csrf_1" });
