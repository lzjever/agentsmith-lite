...
const readOnly: ProjectCapabilities = { canManageEndpoints: false, canManageMembers: false, canManagePolicy: false, canWriteFiles: false, canCreateTasks: false, canSendChat: false };
...
describe("retained chat and overview behavior", () => {
  ...
  it("renders versioned message edit, delete, branch, and only offers retry at the history tail",async()=>{
    ...
    render(<ChatMessageList empty={false} sending={false} messages={[message,completed]} onEdit={(item,content)=>edits.push([item.id,content])} onDelete={(item)=>deleted.push(item.id)} onBranch={(item)=>branched.push(item.id)} onRetry={()=>assert.fail("non-tail retry must not be offered")}/>);assert.ok(screen.getByText("Provider request failed."));assert.equal(screen.queryByRole("button",{name:"Retry"}),null);fireEvent.click(screen.getByRole("button",{name:"Branch from message"}));assert.deepEqual(branched,["message_2"]);fireEvent.click(screen.getByRole("button",{name:"Edit message"}));await screen.findByRole("dialog",{name:"Edit message"});const save=screen.getByRole("button",{name:"Save message"}) as HTMLButtonElement;assert.equal(save.disabled,true);fireEvent.submit(save.closest("form")!);assert.deepEqual(edits,[]);fireEvent.change(screen.getByRole("textbox",{name:"Message text"}),{target:{value:"Revised"}});assert.equal(save.disabled,false);fireEvent.click(save);assert.deepEqual(edits,[["message_1","Revised"]]);fireEvent.click(screen.getAllByRole("button",{name:"Delete message"})[0]!);const dialog=await screen.findByRole("alertdialog",{name:"Delete message"});fireEvent.click(within(dialog).getByRole("button",{name:"Delete message"}));await waitFor(()=>assert.deepEqual(deleted,["message_1"]));
  });

  it("offers retry for the latest failed message and anchors scrolling inside the message viewport", async () => {
    const message={id:"message_1",threadId:"chat_1",sequence:1,version:2,deliveryStatus:"failed" as const,role:"user" as const,content:"Retry me",createdAt:endpoint.createdAt,updatedAt:endpoint.updatedAt};
    const retried:string[]=[];let scrolled:HTMLElement|undefined;const original=HTMLElement.prototype.scrollIntoView;HTMLElement.prototype.scrollIntoView=function(){scrolled=this;};
    try{render(<ChatMessageList empty={false} sending={false} messages={[message]} onRetry={(item)=>retried.push(item.id)}/>);await waitFor(()=>assert.ok(scrolled));assert.match(scrolled!.parentElement?.className??"",/overflow-y-auto/);fireEvent.click(screen.getByRole("button",{name:"Retry"}));assert.deepEqual(retried,["message_1"]);}finally{HTMLElement.prototype.scrollIntoView=original;}
  });
  ...
  it("does not replace the selected conversation when an earlier branch finishes late", async () => {
    const original={endpoints:apiClient.endpoints,projectCapabilities:apiClient.projectCapabilities,chatThreads:apiClient.chatThreads,chatMessages:apiClient.chatMessages,branchChatMessage:apiClient.branchChatMessage};
    const second={...threads[0]!,id:"chat_2",title:"Release notes"};const branch={...threads[0]!,id:"chat_branch",title:"Branched conversation"};let finishBranch!:()=>void;
    apiClient.endpoints=async()=>[endpoint];apiClient.projectCapabilities=async()=>({...readOnly,canSendChat:true});apiClient.chatThreads=async()=>[...threads,second];apiClient.chatMessages=async(_projectId,id)=>[{id:`message_${id}`,threadId:id,sequence:1,version:1,deliveryStatus:"completed",role:"assistant",content:`Message for ${id}`,createdAt:endpoint.createdAt,updatedAt:endpoint.updatedAt}];apiClient.branchChatMessage=async()=>{await new Promise<void>((resolve)=>{finishBranch=resolve;});return branch;};
    try{render(<ProjectChatPage projectId="project_1"/>);await screen.findByText("Message for chat_1");fireEvent.click(screen.getByRole("button",{name:"Branch from message"}));await waitFor(()=>assert.ok(finishBranch));fireEvent.click(screen.getByRole("button",{name:"Release notes"}));await screen.findByText("Message for chat_2");await act(async()=>finishBranch());await waitFor(()=>assert.equal(screen.getByRole("button",{name:"Release notes"}).getAttribute("aria-current"),"true"));assert.ok(screen.getByRole("button",{name:"Branched conversation"}));}
