...
describe("tasks page loading", () => {
  it("describes task work according to the projected create capability", async () => {
    ...
    const readOnly: ProjectCapabilities = { canManageEndpoints: false, canManageMembers: false, canManagePolicy: false, canWriteFiles: false, canCreateTasks: false };
  ...
  it("uses the task list URL as the filter source and restores it on browser navigation", async () => {
    ...
    apiClient.endpoints = async () => [];
    apiClient.fileLibraries = async () => [];
    apiClient.projectCapabilities = async () => ({ canManageEndpoints: false, canManageMembers: false, canManagePolicy: false, canWriteFiles: false, canCreateTasks: false });
  ...
  it("does not navigate to a task created for a project the user has left", async () => {
    ...
    const manager: ProjectCapabilities = { canManageEndpoints: true, canManageMembers: true, canManagePolicy: true, canWriteFiles: true, canCreateTasks: true };
