...
describe("usage page", () => {
  it("renders server-computed limit cards and requests an endpoint-filtered trend", async () => {
    ...
    apiClient.usage = async (_projectId, endpointId) => { requested.push(endpointId); return endpointId === "endpoint_2" ? { ...overviewWithUnassigned, selectedEndpointId: endpointId, daily: overview.daily.map((day) => ({ ...day, requests: 0, tokens: 0, cost: 0 })), trendTotals: { requests: 0, tokens: 0, cost: 0 } } : overviewWithUnassigned; };
    try {
