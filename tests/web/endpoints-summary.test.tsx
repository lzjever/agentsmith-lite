...
describe("endpoint summary",()=>it("shows configured summary and read-only health status",async()=>{const original={endpoints:apiClient.endpoints,credentials:apiClient.credentials,projectCapabilities:apiClient.projectCapabilities};apiClient.endpoints=async()=>[endpoint];apiClient.credentials=async()=>[];apiClient.projectCapabilities=async()=>viewer;try{render(<EndpointsPage projectId="project_1"/>);await screen.findByText("1 endpoint configured");assert.equal(screen.queryByText(/configured · 1 configured/),null);assert.ok(screen.getByText("Read-only access."));assert.ok(screen.getAllByText("Healthy").length>0);}finally{apiClient.endpoints=original.endpoints;apiClient.credentials=original.credentials;apiClient.projectCapabilities=original.projectCapabilities;}}));
describe("endpoint dependencies", () => {
  ...
  it("becomes read-only when the project is archived during endpoint creation", async () => {
    ...
    apiClient.createEndpoint=async()=>{throw new ApiError(409,"Project is archived");};
    try {
      render(<EndpointsPage projectId="project_1" />);
      fireEvent.click((await screen.findAllByRole("button",{name:"Create endpoint"}))[0]!);
      ...
      await act(async()=>{fireEvent.click(within(dialog).getByRole("button",{name:"Save"}));await new Promise(resolve=>setTimeout(resolve,0));});
      ...
      assert.match(screen.getByRole("alert").textContent??"",/now read-only/i);
    } finally { Object.assign(apiClient,original); }
  });
