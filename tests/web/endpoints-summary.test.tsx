...
describe("endpoint credential binding", () => {
  it("shows an unavailable binding when the referenced credential cannot be listed", async () => {
    ...
    apiClient.endpoints=async()=>[endpoint];apiClient.credentials=async()=>[];apiClient.projectCapabilities=async()=>viewer;
    try {
      render(<EndpointsPage projectId="project_1"/>);
      await screen.findByText("1 endpoint configured");
      assert.ok(screen.getAllByText("Credential unavailable").length>0);
...
describe("endpoint dependencies", () => {
  ...
  it("blocks a duplicate endpoint name while allowing an endpoint to keep its own name", async () => {
    ...
    try {
      ...
      assert.match(dialog.className,/max-h-\[calc\(100dvh-2rem\)\]/);
