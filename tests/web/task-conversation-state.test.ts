...
const capabilities: TaskCapabilities = { sendMessage: true, editQueuedMessage: true, abortTurn: true, cancelTask: true, openTerminal: true, editTask:true, retryTask:true, duplicateTask:true, archiveTask:true, deleteTask: true };
...
describe("task conversation state", () => {
  it("applies receipts without allowing an older receipt to replace newer live state", () => {
    ...
    const receipt: TaskMessageReceipt = { messageId: "message_1", disposition: "queued_for_active_run", targetTaskId: "task_1", duplicate: false, queuedMessage: queued("Receipt content", "2026-07-13T00:01:00.000Z"), interaction: interaction(2, "Receipt interaction"), capabilities };
  ...
  it("removes a settled queue entry and exposes only the receipt safe error", () => {
    const accepted: TaskMessageReceipt = { messageId: "message_1", disposition: "accepted_by_active_run", targetTaskId: "task_1", duplicate: false, queuedMessage: null, interaction: interaction(2, "Accepted"), capabilities };
