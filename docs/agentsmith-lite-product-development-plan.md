...
- agent 负责 self-hosted substrates 的配置、安装、重部署和本地目标测试；需要验证核心路径时，可主动运行真实本地 OIDC/K8s e2e 或 visual 检查。它们仍是独立、具体的检查，不作为默认流程、发布 gate 或治理框架。
...
- e2e/visual 如需使用，只能作为 agent/developer 主动选择的、独立、具体产品路径检查，不进入默认测试或发布流程。
...
- 不运行无关 e2e/visual；agent 只在当前核心路径需要真实验证时主动触发。
