...
`POST /api/v1/tasks/{taskId}/turn/abort` derives the service key and calls Botified abort only for the current turn; it does not cancel the task, stop detached work, or clean up the sandbox. `POST /api/v1/tasks/{taskId}/cancel` is the separate whole-task path: it fences delivery, drains artifacts, records cleanup intent, and reaps only that task's app-owned resources. Background work stops through its typed interaction and Botified background-task endpoint. Botified errors are returned as structured errors containing `code`, redacted `message`, retryability, and cursor details when available.
