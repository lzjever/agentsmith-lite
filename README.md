...
Sandbox cleanup is automatic from task startup, cancel, TTL, and the runtime tick. `scripts/deploy/status.sh --env substrate.env` and `scripts/deploy/down.sh --env substrate.env [--dry-run]` remain deployment commands; the product has no cleanup CLI.
