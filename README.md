...
Sandbox release is a user-confirmed product action; there is no idle, TTL, or other automatic release policy. Releasing a Sandbox removes the current Run's app-owned Kubernetes resources while retaining the Task conversation, Botified session, and bound File Library. The next message or Terminal open starts a new Run for the same Task. `scripts/deploy/status.sh --env substrate.env` includes app and sandbox Pods. `scripts/deploy/down.sh --env substrate.env [--dry-run]` removes all app-owned resources, including any sandbox Pods still present when the API stops; the product has no separate cleanup CLI.
