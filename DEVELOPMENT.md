...
`npm test` runs focused unit/service/API/boundary tests. `check:botified-runner` is an opt-in local runner process check: it builds the Node output, starts the vendored Botified binary with `--mock-provider`, observes the mock-provider bash marker through timeline/state, verifies the published file, and calls abort.

`check:botified-runner-image` is the next opt-in layer: it builds `agentsmith-lite/botified-runner:check` from `infra/docker/Dockerfile.botified-runner`, runs the runner container with the mock provider, and checks health/messages/timeline/file download/state/abort over a random loopback port. It requires Docker to pull the Dockerfile base images. When it succeeds, it is runner-container-only and does not exercise Kubernetes scheduling, PVC/JuiceFS behavior, product task artifact flow, `publish_file`, or cancel/reap.

The operator lifecycle e2e and browser visual checks are manual commands, not default test commands. Use `scripts/deploy/render.sh`, `scripts/deploy/apply.sh`, and `scripts/deploy/check-product-workflow.sh` for the concrete app deploy and product workflow path being changed.
