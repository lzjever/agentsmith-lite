...
`npm test` runs focused unit/service/API/boundary tests. `acceptance:botified-runner` is an opt-in local runner process check: it builds the Node output, starts the vendored Botified binary with `--mock-provider`, observes the bash release-smoke marker through timeline/state, and calls abort. It does not prove the runner image, Kubernetes scheduling, JuiceFS PVC behavior, or live artifact flow; those remain external acceptance evidence.
