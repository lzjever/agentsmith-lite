...
export class InMemoryProductStore implements ProductStore {
  ...
  async appendProjectChatMessageIfCurrent(threadId:string,afterMessageId:string|null,message:ProjectChatMessage):Promise<"accepted"|"history_changed"|"request_running">{const history=this.chatMessages.filter((item)=>item.threadId===threadId).sort((a,b)=>a.sequence-b.sequence);if(history.some((item)=>item.deliveryStatus==="pending"||item.deliveryStatus==="response_pending"))return"request_running";if((history.at(-1)?.id??null)!==afterMessageId)return"history_changed";this.chatMessages.push(clone(message));return"accepted";}
