...
export class PostgresProductStore implements ProductStore {
  ...
  async listActiveProjectAlerts(projectId: string): Promise<ActiveProjectAlert[]> { const rows = await this.queryRows<ProjectAlertRow>("select * from project_alerts where project_id=$1 and status='active' order by created_at,id", [projectId]); return rows.map(mapActiveAlert); }
  ...
  async updateProjectAlertDeliveryStatus(projectId: string, id: string, status: ProjectAlert["deliveryStatus"], updatedAt: string): Promise<ProjectAlert | null> { const rows = await this.queryRows<ProjectAlertRow>("update project_alerts set delivery_status=$3, updated_at=$4 where project_id=$1 and id=$2 and type<>'historical_task_failure' returning *", [projectId, id, status, updatedAt]); return rows[0] ? mapAlert(rows[0]) : null; }
...
interface AlertRuleRow { id:string; project_id:string; alert_type:ProjectAlertReadType; name:string; metric:AlertRuleMetric; condition:AlertRuleCondition; threshold:number; window_seconds:number|null; scope_kind:"project"|"endpoint"; endpoint_id:string|null; enabled:boolean; retired_was_enabled:boolean|null; created_at:unknown; updated_at:unknown; }
...
function mapAlertRule(row:AlertRuleRow):ProjectAlertRuleView{
  const common={id:row.id,projectId:row.project_id,name:row.name,metric:row.metric,condition:row.condition,threshold:Number(row.threshold),windowSeconds:row.window_seconds===null?null:Number(row.window_seconds),scope:mapAlertRuleScope(row),createdAt:toIso(row.created_at),updatedAt:toIso(row.updated_at)};
  if(row.alert_type==="historical_task_failure"){
    if(row.enabled||row.retired_was_enabled===null)throw new Error("Historical alert rule row violates its read-only state");
    return{...common,alertType:"historical_task_failure",enabled:false,retiredWasEnabled:row.retired_was_enabled};
  }
  if(row.retired_was_enabled!==null)throw new Error("Active alert rule row has retired state");
  return{...common,alertType:row.alert_type,enabled:row.enabled,retiredWasEnabled:null};
}
function mapActiveAlertRule(row:AlertRuleRow):ActiveProjectAlertRuleView{const rule=mapAlertRule(row);if(!isActiveProjectAlertRuleView(rule))throw new Error("Expected an active alert rule row");return rule}
...
function mapAlert(row:ProjectAlertRow):ProjectAlert{
  ...
  if(row.type==="historical_task_failure"){
    if(row.status==="active")throw new Error("Historical project alert row cannot be active");
    return{...fields,type:"historical_task_failure",status:row.status};
  }
