...
export class PostgresProductStore implements ProductStore {
  ...
  async queryProjectAlerts(projectId:string,query:import("../../ports/src/store.js").ProjectAlertStoreQuery):Promise<import("../../ports/src/store.js").ProjectAlertStorePage>{return transaction(this.pool,async(client)=>{const values:unknown[]=[projectId];const where=["project_id=$1",query.view==="active"?"status='active'":"status in ('resolved','dismissed')"];if(query.after){values.push(query.after.createdAt,query.after.id);where.push(`(created_at < $${values.length-1}::timestamptz or (created_at = $${values.length-1}::timestamptz and id collate "C" < $${values.length}::text collate "C"))`)}values.push(query.limit+1);const rows=(await client.query<ProjectAlertRow>(`select * from project_alerts where ${where.join(" and ")} order by created_at desc,id collate "C" desc limit $${values.length}`,values)).rows;const activeCount=Number((await client.query<{count:string}>("select count(*)::text as count from project_alerts where project_id=$1 and status='active'",[projectId])).rows[0]?.count??0);return{items:rows.slice(0,query.limit).map(mapAlert),hasMore:rows.length>query.limit,activeCount}},"repeatable read")}
