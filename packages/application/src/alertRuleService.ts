...
const types:readonly ProjectAlertType[]=["sandbox_capacity","provider_requests_limit","provider_tokens_limit","provider_cost_limit","project_file_bytes_limit","endpoint_failure","provider_failure","sandbox_failure"];
const metrics:readonly AlertRuleMetric[]=["active_sandboxes","provider_requests","provider_tokens","provider_cost","project_file_bytes","failure_count"];
...
export class AlertRuleService {
  ...
  async update(userId:string,projectId:string,id:string,input:Input,idempotencyKey?:string){
    ...
    const update=async()=>{
      const current=requireRule(await this.store.findProjectAlertRule(projectId,id));
      ...
      const updated:ProjectAlertRule={...current,name:input.name===undefined?(current.name??defaultRuleName(type)):text(input.name,"name"),alertType:type,metric:selectedMetric,condition:"greater_than_or_equal",threshold:input.threshold===undefined?(current.threshold??1):number(input.threshold,"threshold"),windowSeconds:input.windowSeconds===undefined?(typeChanged||metricChanged?defaultWindow(selectedMetric):(current.windowSeconds??defaultWindow(selectedMetric))):window(input.windowSeconds,selectedMetric),scope:selectedScope,enabled:input.enabled===undefined?current.enabled:boolean(input.enabled,"enabled"),updatedAt:nextTimestamp(current.updatedAt)};
      const saved=await this.store.updateProjectAlertRule(updated,expected);
      if(!saved){if(await this.store.findProjectAlertRule(projectId,id))throw new ProductError("Alert rule changed elsewhere. Reload and try again.",409);throw new NotFoundError("Alert rule not found")}
...
function defaultRuleName(type:ProjectAlertType){return type==="sandbox_capacity"?"Sandbox capacity":type.replaceAll("_"," ")}
function defaultMetric(type:ProjectAlertType):AlertRuleMetric{return type==="sandbox_capacity"?"active_sandboxes":type==="provider_requests_limit"?"provider_requests":type==="provider_tokens_limit"?"provider_tokens":type==="provider_cost_limit"?"provider_cost":type==="project_file_bytes_limit"?"project_file_bytes":"failure_count"}
...
function window(value:unknown,selectedMetric:AlertRuleMetric){const windowed=isWindowedMetric(selectedMetric);if(value===undefined)return defaultWindow(selectedMetric);if(!windowed){if(value===null)return null;throw new ProductError("Gauge alert rules do not support windowSeconds")}if(value===null)throw new ProductError("windowSeconds is required for windowed alert rules");if(typeof value!=="number"||!Number.isInteger(value)||value<60||value>2592000)throw new ProductError("windowSeconds must be between 60 and 2592000");return value}
function defaultWindow(selectedMetric:AlertRuleMetric):number|null{return isWindowedMetric(selectedMetric)?3600:null}
function isWindowedMetric(selectedMetric:AlertRuleMetric):boolean{return selectedMetric!=="active_sandboxes"&&selectedMetric!=="project_file_bytes"}
