...
export class WorkspaceMembershipService {
  ...
  async add(actor:string,workspaceId:string,identity:Identity,role:ManagedWorkspaceMembershipRole){await this.authorization.requireWorkspace(actor,workspaceId,"admin");const user=await this.find(identity);if(await this.store.findWorkspaceMembership(workspaceId,user.id))throw new ProductError("Workspace membership already exists",409);const timestamp=nowIso();return this.store.upsertWorkspaceMembership({workspaceId,userId:user.id,role,createdAt:timestamp,updatedAt:timestamp})}
