...
export class WorkspaceMembershipService {
  ...
  async add(actor:string,workspaceId:string,identity:Identity,role:ManagedWorkspaceMembershipRole){await this.authorization.requireWorkspace(actor,workspaceId,"admin");const user=await this.find(identity);if(await this.store.findWorkspaceMembership(workspaceId,user.id))throw new ProductError("Workspace membership already exists",409);const timestamp=nowIso();await this.store.upsertWorkspaceMembership({workspaceId,userId:user.id,role,createdAt:timestamp,updatedAt:timestamp});return this.view(workspaceId,user.id)}
  async change(actor:string,workspaceId:string,userId:string,role:ManagedWorkspaceMembershipRole){await this.authorization.requireWorkspace(actor,workspaceId,"admin");const current=await this.store.findWorkspaceMembership(workspaceId,userId);if(!current)throw new NotFoundError("Workspace membership not found");if(current.role==="owner")throw new ProductError("Workspace owner membership cannot be changed or removed",409);if(!await this.store.updateWorkspaceMembership({...current,role,updatedAt:nowIso()}))throw new NotFoundError("Workspace membership not found");return this.view(workspaceId,userId)}
