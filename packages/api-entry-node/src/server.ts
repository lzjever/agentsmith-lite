...
function asAuditQuery(params:URLSearchParams):import("../../contracts/src/api.js").ProjectAuditQuery{const value:import("../../contracts/src/api.js").ProjectAuditQuery={};for(const key of ["actorId","subjectUserId"] as const){const id=params.get(key);if(id!==null){if(!id||id.length>128||/[\u0000-\u001f\u007f]/.test(id))throw new ProductError(`Audit ${key} is invalid`);value[key]=id==="system"?null:id}}const limit=params.get("limit");if(limit!==null){const parsed=Number(limit);if(!Number.isInteger(parsed)||parsed<1||parsed>100)throw new ProductError("Audit limit must be between 1 and 100");value.limit=parsed}const cursor=params.get("cursor");if(cursor){const split=cursor.lastIndexOf("|");if(split<1||!Number.isFinite(Date.parse(cursor.slice(0,split)))||!cursor.slice(split+1))throw new ProductError("Audit cursor is invalid");value.cursor=cursor}for(const key of ["from","to"] as const){const item=params.get(key);if(item){if(!Number.isFinite(Date.parse(item)))throw new ProductError(`Audit ${key} timestamp is invalid`);value[key]=new Date(item).toISOString()}}const action=params.get("action");if(action){if(!auditActions.has(action))throw new ProductError("Audit action is invalid");value.action=action as import("../../contracts/src/api.js").ProjectAuditAction}const status=params.get("status");if(status==="accepted"||status==="rejected")value.status=status;else if(status)throw new ProductError("Audit status is invalid");const kind=params.get("resourceKind");if(kind){if(!auditResourceKinds.has(kind))throw new ProductError("Audit resource kind is invalid");value.resourceKind=kind as import("../../contracts/src/api.js").ProjectAuditResourceKind}const resourceId=params.get("resourceId");if(resourceId!==null){if(!resourceId||resourceId.length>1024||/[\u0000-\u001f\u007f]/.test(resourceId))throw new ProductError("Audit resource ID is invalid");value.resourceId=resourceId}return value}
