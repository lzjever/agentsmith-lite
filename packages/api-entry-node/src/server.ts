...
async function routeApi(
  ...
  if (segments[0] === "api" && segments[1] === "v1" && segments[2] === "projects" && segments[3]) {
    ...
    if (segments[4] === "alerts") {
      if (!segments[5] && method === "GET") { assertOnlySearchParams(url,["view","limit","cursor"]);return sendJson(res, 200, await services.policies.alerts(user.id, projectId, asProjectAlertQuery(url.searchParams))); }
    ...
    if (segments[4] === "alert-rules") {
      ...
      if (segments[5] && !segments[6] && method === "GET") { assertOnlySearchParams(url,[]);return sendJson(res,200,await services.alertRules.find(user.id,projectId,segments[5])); }
    ...
    if (segments[4] === "audit" && !segments[5] && method === "GET") { assertOnlySearchParams(url,["actorId","subjectUserId","limit","cursor","from","to","action","status","resourceKind","resourceId"]);return sendJson(res, 200, await services.policies.audit(user.id, projectId, asAuditQuery(url.searchParams))); }
    if(segments[4]==="audit"&&segments[5]==="identities"&&!segments[6]&&method==="GET"){assertOnlySearchParams(url,["role","q","cursor","limit"]);return sendJson(res,200,await services.policies.auditIdentities(user.id,projectId,asAuditIdentityQuery(url.searchParams)))}
    if(segments[4]==="file-libraries"){
      ...
      if(libraryId&&segments[6]==="files"){
        ...
        if(!action&&method==="PUT"){
          ...
          const saved=await services.settings.runIdempotentMutation(user.id,projectId,"project.file.upload",key,{projectId,libraryId,filePath,overwrite,contentSha256:createHash("sha256").update(bytes).digest("base64url")},`${libraryId}:${filePath}`,()=>services.fileLibraries.withLibraryMutation(user.id,projectId,libraryId,({library,projectRoot,rootSubPaths})=>services.files.uploadLibraryFileWithAccounting(projectRoot,library.rootSubPath,{path:filePath,bytes,overwrite},{rootSubPaths,reconcile:async(measured)=>{await services.policies.reconcileFileLibraryBytes(projectId,measured)},reserve:(storedPath,delta)=>services.policies.reserveFileUpload(projectId,user.id,`${library.rootSubPath}/${storedPath}`,delta),committed:(storedPath,file)=>services.policies.commitFileUpload(projectId,user.id,library.id,`${library.rootSubPath}/${storedPath}`,file.bytes,file.mediaType)})));
...
function optionalUserId(params:URLSearchParams,name:string):string|undefined{const value=params.get(name);if(value===null)return undefined;if(!value||value.length>128||/[\u0000-\u001f\u007f]/.test(value))throw new ProductError(`${name} query parameter is invalid`);return value}
...
function asAuditQuery(params:URLSearchParams):import("../../contracts/src/api.js").ProjectAuditQuery{const value:import("../../contracts/src/api.js").ProjectAuditQuery={};const actorId=params.get("actorId");if(actorId!==null){if(!actorId||actorId.length>128||/[\u0000-\u001f\u007f]/.test(actorId))throw new ProductError("Audit actorId is invalid");value.actorId=actorId==="system"?null:actorId}const subjectUserId=params.get("subjectUserId");if(subjectUserId!==null){if(!subjectUserId||subjectUserId==="system"||subjectUserId.length>128||/[\u0000-\u001f\u007f]/.test(subjectUserId))throw new ProductError("Audit subjectUserId is invalid");value.subjectUserId=subjectUserId}const limit=params.get("limit");if(limit!==null){const parsed=Number(limit);if(!Number.isInteger(parsed)||parsed<1||parsed>100)throw new ProductError("Audit limit must be between 1 and 100");value.limit=parsed}const cursor=params.get("cursor");if(cursor!==null)value.cursor=cursor;for(const key of ["from","to"] as const){const item=params.get(key);if(item!==null){if(!Number.isFinite(Date.parse(item)))throw new ProductError(`Audit ${key} timestamp is invalid`);value[key]=new Date(item).toISOString()}}const action=params.get("action");if(action!==null){if(!auditActions.has(action))throw new ProductError("Audit action is invalid");value.action=action as import("../../contracts/src/api.js").ProjectAuditAction}const status=params.get("status");if(status==="accepted"||status==="rejected")value.status=status;else if(status!==null)throw new ProductError("Audit status is invalid");const kind=params.get("resourceKind");if(kind!==null){if(!auditResourceKinds.has(kind))throw new ProductError("Audit resource kind is invalid");value.resourceKind=kind as import("../../contracts/src/api.js").ProjectAuditResourceKind}const resourceId=params.get("resourceId");if(resourceId!==null){if(!resourceId||resourceId.length>1024||/[\u0000-\u001f\u007f]/.test(resourceId))throw new ProductError("Audit resource ID is invalid");value.resourceId=resourceId}return value}
function asAuditIdentityQuery(params:URLSearchParams):import("../../contracts/src/api.js").ProjectAuditIdentityQuery{const role=params.get("role");if(role!=="actor"&&role!=="subject")throw new ProductError("Audit identity role must be actor or subject");const q=params.get("q")??"";if(q.trim().length>120||/[\u0000-\u001f\u007f]/.test(q))throw new ProductError("Audit identity query is invalid");const value:import("../../contracts/src/api.js").ProjectAuditIdentityQuery={role,...(q?{q}:{})};const cursor=params.get("cursor");if(cursor!==null)value.cursor=cursor;const limit=params.get("limit");if(limit!==null){const parsed=Number(limit);if(!Number.isInteger(parsed)||parsed<1||parsed>50)throw new ProductError("Audit identity limit must be between 1 and 50");value.limit=parsed}return value}

function asProjectAlertQuery(params:URLSearchParams):import("../../contracts/src/api.js").ProjectAlertQuery{const value:import("../../contracts/src/api.js").ProjectAlertQuery={};const view=params.get("view");if(view==="active"||view==="history")value.view=view;else if(view!==null)throw new ProductError("Alert view is invalid");const limit=params.get("limit");if(limit!==null){const parsed=Number(limit);if(!Number.isInteger(parsed)||parsed<1||parsed>50)throw new ProductError("Alert limit must be between 1 and 50");value.limit=parsed}const cursor=params.get("cursor");if(cursor===null)return value;if(cursor.length<1)throw new ProductError("Alert cursor is invalid");value.cursor=cursor;return value}
