...
function asProjectAlertQuery(params:URLSearchParams):import("../../contracts/src/api.js").ProjectAlertQuery{const value:import("../../contracts/src/api.js").ProjectAlertQuery={};const limit=params.get("limit");if(limit!==null){const parsed=Number(limit);if(!Number.isInteger(parsed)||parsed<1||parsed>100)throw new ProductError("Alert limit must be between 1 and 100");value.limit=parsed}const cursor=params.get("cursor");if(cursor){const split=cursor.lastIndexOf("|");if(split<1||!Number.isFinite(Date.parse(cursor.slice(0,split)))||!cursor.slice(split+1))throw new ProductError("Alert cursor is invalid");value.cursor=cursor}const status=params.get("status");if(status==="active"||status==="resolved"||status==="dismissed")value.status=status;else if(status)throw new ProductError("Alert status is invalid");return value}
